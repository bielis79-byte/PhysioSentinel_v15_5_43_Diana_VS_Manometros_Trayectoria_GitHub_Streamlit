...
# v15.5.15 · exportación de vídeo/GIF del atractor
try:
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    import imageio.v2 as imageio
    VIDEO_ATTRACTOR_AVAILABLE = True
except Exception:
    plt = None
    imageio = None
    VIDEO_ATTRACTOR_AVAILABLE = False
...
APP_VERSION = "v15.5.43"
APP_DOCUMENTED_REVISIONS = 60
APP_VERSION_NOTE = f"{APP_VERSION} · ≥{APP_DOCUMENTED_REVISIONS} revisiones/versiones históricas documentadas en el código actual"
...
def _ctx_metric_score_v15534(value, low, high, sex_adj=0.0):
    """Score 0-100 contextualizado.
    50 = límite inferior del rango esperado, 75 = límite superior.
    Dentro del rango nunca se etiqueta como bajo por estar bajo el centro.
    """
    try:
        v=float(value)
        if not np.isfinite(v) or high <= low:
        ...
        w=high-low
        if v < low:
            s = 50.0 * max(0.0, 1.0 - (low-v)/w)
        elif v <= high:
            s = 50.0 + 25.0*(v-low)/w
        else:
            s = 75.0 + 25.0*min((v-high)/w, 1.0)
        return float(np.clip(s + sex_adj, 0.0, 100.0))
    except Exception:
        return np.nan


def _ctx_optimal_score_v15534(value, low, high):
    """Rango funcional: 75 dentro del intervalo; cae al alejarse."""
    try:
        v=float(value)
        if not np.isfinite(v) or high <= low:
            return np.nan
        if low <= v <= high:
            return 75.0
        w=high-low
        d=(low-v) if v < low else (v-high)
        return float(np.clip(75.0 - 50.0*(d/w), 0.0, 75.0))
    except Exception:
        return np.nan


def _ctx_abnormal_high_score_v15534(value, low, high):
    """Para rigidez: bajo dentro/debajo del rango, aumenta al superar el límite superior."""
    try:
        v=float(value)
        if not np.isfinite(v) or high <= low:
            return np.nan
        if v <= low:
            return 20.0
        if v <= high:
            return 20.0 + 30.0*(v-low)/(high-low)
        return float(np.clip(50.0 + 50.0*(v-high)/(high-low), 0.0, 100.0))
    except Exception:
        return np.nan
...
def _load_all_reference_profiles_v15535(db_path_str):
    """Carga perfiles generales y overrides por registro de una sola vez."""
    general, overrides = {}, {}
    try:
        _ensure_patient_reference_profile_table_v15528()
        _ensure_record_reference_profile_table_v15533()
        with sqlite3.connect(db_path_str) as con:
            try:
                for pid, age, sex, updated in con.execute(
                    "SELECT patient_id, age_years, sex, updated_at FROM patient_reference_profile_v15528"
                ).fetchall():
                    general[str(pid)] = {
                        "age_years": age, "sex": sex or "No especificado",
                        "updated_at": updated, "source": "paciente"
                    }
            except Exception:
                pass
            try:
                for pid, rid, age, sex, updated in con.execute(
                    "SELECT patient_id, record_id, age_years, sex, updated_at FROM record_reference_profile_v15533"
                ).fetchall():
                    overrides[(str(pid), str(rid))] = {
                        "age_years": age, "sex": sex or "No especificado",
                        "updated_at": updated, "source": "registro"
                    }
            except Exception:
                pass
    except Exception:
...
def _resolve_profile_from_maps_v15535(patient_id, record_id, maps):
    general, overrides = maps
    pid = infer_patient_id(patient_id)
    rid = str(record_id or "")
    if (pid, rid) in overrides:
        return overrides[(pid, rid)]
    return general.get(pid, {"age_years": None, "sex": "No especificado", "source": "ninguno"})


def _ctx_signature_v15535(row, profile):
    """Firma estable de métricas relevantes + perfil; evita recalcular si nada cambia."""
    keys = [
        'RMSSD','SD1','HF','pNN50','HF_DOM_PCT','HF_LS',
        'SDNN','SD2','TOTAL','TOTAL_LS',
        'SampEn','DispEn','D2','Lyapunov_LLE',
        'DET','Lmax','Hurst','WAV_TRANSITIONS_PER_MIN',
        'VLF','VLF_DOM_PCT','VLF_LS','DFA_alpha1','DFA_alpha2',
        'WAV_ENTROPY_BANDS','WAV_ENTROPY_GLOBAL'
    ]
    payload = {
        "record": str(row.get('Registro','')),
        "patient": infer_patient_id(str(row.get('Paciente_ID','') or row.get('Registro',''))),
    ...
    for k in keys:
        try:
            v = row.get(k, np.nan)
            payload[k] = None if not np.isfinite(float(v)) else round(float(v), 8)
        except Exception:
            payload[k] = None
...
def _physiological_indices_contextual_cached_v15535(signature, row_payload, profile_payload):
    """Cache por registro/perfil. signature invalida automáticamente cambios."""
    row = pd.Series(row_payload)
    profile = dict(profile_payload)

    # Monkey-resolve simple: usa directamente el perfil pasado, sin consultar SQLite.
    get=lambda k: row.get(k, np.nan)
    age=profile.get('age_years')
    sex=profile.get('sex','No especificado')
    grp=_age_group_v15528(age)
    norms=AUTONOMIC_AGE_NORMS_V15528.get(grp, {}) if grp else {}
    if not norms:
        return {
            **{f"{k}_CTX":np.nan for k in [
                'IDX_Vagal','IDX_Amplitud','IDX_Complejidad','IDX_Rigidez',
                'IDX_Adaptabilidad','IDX_Regulacion_Lenta'
            ]},
            'IDX_Equilibrio_Regulacion_Lenta_CTX':np.nan,
            'IDX_CTX_Disponible':False,'IDX_CTX_Edad':age,'IDX_CTX_Sexo':sex,
            'IDX_CTX_GrupoEdad':grp,'IDX_CTX_FuentePerfil':profile.get('source','ninguno'),
        }

    def age_score(metric, default=np.nan):
        if metric not in norms: return default
        val=pd.to_numeric(get(metric), errors='coerce')
        if not np.isfinite(val): return np.nan
        lo,hi=norms[metric]
        adj=0.0
        try:
            adj=25.0*_sex_context_adjustment_v15528(metric, sex)
        except Exception:
            pass
        return _ctx_metric_score_v15534(val, lo, hi, adj)

    vagal=_weighted_mean_available([
        (age_score('RMSSD'),1.4),(age_score('SD1'),1.2),
        (age_score('HF'),1.1),(age_score('pNN50'),0.8),
        (_clip_score(get('HF_DOM_PCT'),10,65),0.6),
        (_log_score(get('HF_LS'),20,1200),0.3),
    ])

    amplitude=_weighted_mean_available([
        (age_score('SDNN'),1.3),(age_score('SD2'),1.1),
        (age_score('TOTAL'),1.0),(_log_score(get('TOTAL_LS'),80,4000),0.25),
    ])

    static=AUTONOMIC_STATIC_NORMS_V15528
    samp=age_score('SampEn')
    d2=_ctx_metric_score_v15534(get('D2'), *static.get('D2',(2.5,4.5))) if np.isfinite(pd.to_numeric(get('D2'),errors='coerce')) else np.nan
    disp=_ctx_metric_score_v15534(get('DispEn'), *static.get('DispEn',(1.8,2.6))) if np.isfinite(pd.to_numeric(get('DispEn'),errors='coerce')) else np.nan
    lle=_optimal_score(get('Lyapunov_LLE'),0.10,0.10)
    complexity=_weighted_mean_available([
        (samp,1.2),(disp,1.0),(d2,0.8),(lle,0.5),
        (_multiscale_area_score(row,'MDE',20),0.8),
        (_multiscale_area_score(row,'MSE',20),0.5),
    ])

    det=_ctx_abnormal_high_score_v15534(get('DET'), *static.get('DET',(60,85))) if np.isfinite(pd.to_numeric(get('DET'),errors='coerce')) else np.nan
    lmax=_ctx_abnormal_high_score_v15534(get('Lmax'), *static.get('Lmax',(15,50))) if np.isfinite(pd.to_numeric(get('Lmax'),errors='coerce')) else np.nan
    hurst_val=pd.to_numeric(get('Hurst'),errors='coerce')
    if np.isfinite(hurst_val):
        hlo,hhi=static.get('Hurst',(0.65,0.85))
        hurst_rig=25.0 if hlo <= hurst_val <= hhi else float(np.clip(25+50*min(abs(hurst_val-(hlo if hurst_val<hlo else hhi))/(hhi-hlo),1),0,100))
    else:
        hurst_rig=np.nan
    transitions=_clip_score(get('WAV_TRANSITIONS_PER_MIN'),0.2,2.5)
    rigidity=_weighted_mean_available([
        (det,1.2),(lmax,0.8),(hurst_rig,0.7),
        ((100-complexity) if np.isfinite(complexity) else np.nan,1.0),
        ((100-transitions) if np.isfinite(transitions) else np.nan,0.5),
    ])

    # Regulación lenta contextual · v15.5.36
    # IDX_Regulacion_Lenta_CTX = INTENSIDAD/CARGA de componente lento, no calidad.
    # Valores altos significan mayor expresión VLF/lenta; no se consideran automáticamente mejores.
    vlf=age_score('VLF')
    dfa2=_ctx_optimal_score_v15534(get('DFA_alpha2'), *static.get('DFA_alpha2',(0.90,1.10))) if np.isfinite(pd.to_numeric(get('DFA_alpha2'),errors='coerce')) else np.nan
    hurst_opt=_ctx_optimal_score_v15534(get('Hurst'), *static.get('Hurst',(0.65,0.85))) if np.isfinite(hurst_val) else np.nan
    slow=_weighted_mean_available([
        (vlf,1.0),
        (_clip_score(get('VLF_DOM_PCT'),15,65),0.6),
        (_log_score(get('VLF_LS'),20,1500),0.25),
    ])

    # Equilibrio lento: dentro del rango de VLF por edad + DFAα2/Hurst organizados.
    # Aquí sí: mayor = más equilibrado/preservado.
    try:
        _vlo,_vhi = norms.get('VLF',(np.nan,np.nan))
        vlf_balance = _ctx_optimal_score_v15534(get('VLF'), _vlo, _vhi) if np.isfinite(_vlo) and np.isfinite(_vhi) else np.nan
    except Exception:
        vlf_balance=np.nan

    vlf_dom_val=pd.to_numeric(get('VLF_DOM_PCT'),errors='coerce')
    if np.isfinite(vlf_dom_val):
        # Dominancia VLF muy alta penaliza equilibrio; valores bajos/moderados no se premian como "mejores" sin límite.
        if vlf_dom_val <= 45:
            vlf_dom_balance=75.0
        else:
            vlf_dom_balance=float(np.clip(75.0 - 2.0*(vlf_dom_val-45.0), 20.0, 75.0))
    else:
        vlf_dom_balance=np.nan

    slow_balance=_weighted_mean_available([
        (vlf_balance,1.0),
        (vlf_dom_balance,0.55),
        (dfa2,0.45),
        (hurst_opt,0.35),
    ])

    dfa1=_ctx_optimal_score_v15534(get('DFA_alpha1'), *static.get('DFA_alpha1',(0.85,1.15))) if np.isfinite(pd.to_numeric(get('DFA_alpha1'),errors='coerce')) else np.nan
    wave_entropy=_weighted_mean_available([
        (_clip_score(get('WAV_ENTROPY_BANDS'),0.25,0.90),1.0),
        (_clip_score(get('WAV_ENTROPY_GLOBAL'),0.25,0.90),0.8),
    ])
    adaptability=_weighted_mean_available([
        (vagal,1.0),(amplitude,0.8),(complexity,1.2),
        (dfa1,0.9),(wave_entropy,0.6),(transitions,0.5),
        ((100-rigidity) if np.isfinite(rigidity) else np.nan,1.0),
    ])

    return {
        'IDX_Vagal_CTX':vagal,'IDX_Amplitud_CTX':amplitude,
        'IDX_Complejidad_CTX':complexity,'IDX_Rigidez_CTX':rigidity,
        'IDX_Adaptabilidad_CTX':adaptability,
        'IDX_Regulacion_Lenta_CTX':slow,
        'IDX_Equilibrio_Regulacion_Lenta_CTX':slow_balance,
        'IDX_Equilibrio_Regulacion_Lenta_CTX':slow_balance,
        'IDX_CTX_Disponible':True,'IDX_CTX_Edad':age,'IDX_CTX_Sexo':sex,
        'IDX_CTX_GrupoEdad':grp,'IDX_CTX_FuentePerfil':profile.get('source','paciente'),
    }


def physiological_indices_contextual_fast_v15535(row, maps):
    profile = _resolve_profile_from_maps_v15535(
        str(row.get('Paciente_ID','') or row.get('Registro','')),
        str(row.get('Registro','')),
        maps
    )
    sig = _ctx_signature_v15535(row, profile)
    payload = {}
    for k,v in row.items():
        if isinstance(v, (np.generic,)):
            v = v.item()
        payload[k] = v
    return _physiological_indices_contextual_cached_v15535(sig, payload, profile)


def physiological_indices_contextual_from_row_v15534(row):
    """Segunda capa de índices 0-100 contextualizados por edad/sexo.
    No modifica los índices históricos/brutos.
    """
    get=lambda k: row.get(k, np.nan)
    patient_id = str(row.get('Paciente_ID','') or row.get('Registro',''))
    record_id = str(row.get('Registro',''))
    try:
        prof = _resolve_reference_profile_v15533(patient_id, record_id)
    except Exception:
        prof = {"age_years":None, "sex":"No especificado", "source":"ninguno"}

    age=prof.get('age_years')
    sex=prof.get('sex','No especificado')
    try:
        grp=_age_group_v15528(age)
        norms=AUTONOMIC_AGE_NORMS_V15528.get(grp, {}) if grp else {}
    except Exception:
    ...
    raw=physiological_indices_from_row(row)
    if not norms:
        return {
            **{f"{k}_CTX":np.nan for k in [
                'IDX_Vagal','IDX_Amplitud','IDX_Complejidad','IDX_Rigidez',
            ...
            'IDX_CTX_Disponible':False,
            'IDX_CTX_Edad':age,
            'IDX_CTX_Sexo':sex,
            'IDX_CTX_GrupoEdad':grp,
            'IDX_CTX_FuentePerfil':prof.get('source','ninguno'),
        }
    ...
    def age_score(metric, default=np.nan):
        ...
        adj=0.0
        try:
            # ajuste de sexo muy suave: ±2.5 puntos como máximo
            adj=25.0*_sex_context_adjustment_v15528(metric, sex)
        except Exception:
            pass
        return _ctx_metric_score_v15534(val, lo, hi, adj)
    ...
    amplitude=_weighted_mean_available([
        ...
        (age_score('SD2'),1.1),
        (age_score('TOTAL'),1.0),
        (_log_score(get('TOTAL_LS'),80,4000),0.25),
    ])

    # Complejidad: SampEn por edad, resto con referencias estructurales disponibles
    try:
        static=AUTONOMIC_STATIC_NORMS_V15528
    except Exception:
        static={}
    ...
    # Rigidez contextual: DET/Lmax/Hurst estructurales + complejidad inversa/movilidad
    det=_ctx_abnormal_high_score_v15534(get('DET'), *static.get('DET',(60,85))) if np.isfinite(pd.to_numeric(get('DET'),errors='coerce')) else np.nan
    lmax=_ctx_abnormal_high_score_v15534(get('Lmax'), *static.get('Lmax',(15,50))) if np.isfinite(pd.to_numeric(get('Lmax'),errors='coerce')) else np.nan
    hurst_val=pd.to_numeric(get('Hurst'),errors='coerce')
    if np.isfinite(hurst_val):
    ...
    vlf_dom_val=pd.to_numeric(get('VLF_DOM_PCT'),errors='coerce')
    if np.isfinite(vlf_dom_val):
        vlf_dom_balance=75.0 if vlf_dom_val <= 45 else float(np.clip(75.0-2.0*(vlf_dom_val-45.0),20.0,75.0))
    else:
        vlf_dom_balance=np.nan
    slow_balance=_weighted_mean_available([
        (vlf_balance,1.0),(vlf_dom_balance,0.55),(dfa2,0.45),(hurst_opt,0.35)
    ])

    # Adaptabilidad: se deriva de componentes YA contextualizados, sin segunda normalización por edad
    dfa1=_ctx_optimal_score_v15534(get('DFA_alpha1'), *static.get('DFA_alpha1',(0.85,1.15))) if np.isfinite(pd.to_numeric(get('DFA_alpha1'),errors='coerce')) else np.nan
    wave_entropy=_weighted_mean_available([
        (_clip_score(get('WAV_ENTROPY_BANDS'),0.25,0.90),1.0),
        (_clip_score(get('WAV_ENTROPY_GLOBAL'),0.25,0.90),0.8),
    ...
    return {
        'IDX_Vagal_CTX':vagal,
        'IDX_Amplitud_CTX':amplitude,
        'IDX_Complejidad_CTX':complexity,
        'IDX_Rigidez_CTX':rigidity,
        'IDX_Adaptabilidad_CTX':adaptability,
        'IDX_Regulacion_Lenta_CTX':slow,
        'IDX_CTX_Disponible':True,
        'IDX_CTX_Edad':age,
        'IDX_CTX_Sexo':sex,
        'IDX_CTX_GrupoEdad':grp,
        'IDX_CTX_FuentePerfil':prof.get('source','paciente'),
    }


def _indices_mode_frame_v15534(df, mode):
    """Devuelve una copia para las gráficas; en modo contextual sustituye visualmente los IDX."""
    if df is None or df.empty:
        return pd.DataFrame()
    out=df.copy()
    if mode == "Contextualizado por edad/sexo":
        for base in [
            'IDX_Vagal','IDX_Amplitud','IDX_Complejidad','IDX_Rigidez',
            'IDX_Adaptabilidad','IDX_Regulacion_Lenta'
        ]:
            ...
            if ctx in out.columns:
                # fallback bruto por fila cuando aún no existe perfil contextual
                out[base]=pd.to_numeric(out[ctx],errors='coerce').combine_first(
                    pd.to_numeric(out.get(base),errors='coerce')
                )
        out['Perfil_autonomico']=out.apply(
            lambda r: autonomic_profile_from_indices(r), axis=1
        )
    return out


def _contextualize_indices_dataframe_v15535(df, maps=None):
    """Contextualiza bajo demanda usando cache por registro.
    maps evita abrir SQLite por cada fila.
    """
    if df is None or df.empty:
        return pd.DataFrame() if df is None else df.copy()
    if maps is None:
        maps = _load_all_reference_profiles_v15535(str(LONGITUDINAL_DB_PATH))
    rows=[]
    for _,r in df.iterrows():
        d=r.to_dict()
        try:
            d.update(physiological_indices_contextual_fast_v15535(r, maps))
        except Exception:
            pass
        rows.append(d)
    return pd.DataFrame(rows)
...
def _profile_maps_signature_v15539(maps):
    general, overrides = maps
    parts=[]
    for pid,p in sorted(general.items(), key=lambda x:str(x[0])):
        parts.append(("P",str(pid),p.get("age_years"),p.get("sex"),p.get("updated_at")))
    for key,p in sorted(overrides.items(), key=lambda x:(str(x[0][0]),str(x[0][1]))):
        parts.append(("R",str(key[0]),str(key[1]),p.get("age_years"),p.get("sex"),p.get("updated_at")))
    return hashlib.sha256(repr(parts).encode("utf-8")).hexdigest()


@st.cache_data(show_spinner=False, max_entries=64)
def _contextualize_indices_bulk_cached_v15539(df, profile_signature, general, overrides):
    """Cache a nivel de DataFrame + perfil. Evita iterar todas las filas en cada rerun."""
    maps=(general,overrides)
    return _contextualize_indices_dataframe_v15535(df, maps)


def _contextualize_indices_bulk_v15539(df, maps):
    if df is None or df.empty:
        return pd.DataFrame() if df is None else df.copy()
    sig=_profile_maps_signature_v15539(maps)
    return _contextualize_indices_bulk_cached_v15539(df, sig, maps[0], maps[1])


def _fast_df_signature_v15539(df, columns=None):
    """Firma rápida pero sensible a las métricas que afectan a pestañas 9/10."""
    if df is None or not isinstance(df,pd.DataFrame) or df.empty:
        return "empty"
    if columns is None:
        columns=[
            'Paciente_ID','Registro','Fase','Fecha_hora',
            'RMSSD','SDNN','SD1','SD2','HF','LF','VLF','TOTAL','LF_HF',
            'HF_DOM_PCT','LF_DOM_PCT','VLF_DOM_PCT','MeanHR','pNN50',
            'DFA_alpha1','DFA_alpha2','Hurst','SampEn','DispEn','D2',
            'Lyapunov_LLE','DET','Lmax','IDX_Vagal','IDX_Amplitud',
            'IDX_Complejidad','IDX_Rigidez','IDX_Regulacion_Lenta',
            'WAV_ENTROPY_GLOBAL','WAV_TRANSITIONS_PER_MIN',
            'HVG_graph_score_small_world','HVG_clustering'
        ]
    ...
    try:
        ...
        return hashlib.sha256(
            (str(df.shape)+"|"+repr(df[cols].astype(str).head(500).to_dict("records"))).encode("utf-8")
        ).hexdigest()


def _autonomic_cache_signature_v15539(long_df, indices_df, history_df, profile_maps, record_data=None):
    parts=[
        _fast_df_signature_v15539(long_df),
        _fast_df_signature_v15539(indices_df),
        _fast_df_signature_v15539(history_df),
        _profile_maps_signature_v15539(profile_maps),
    ]
    if isinstance(record_data,dict):
        parts.append("|".join(sorted(map(str,record_data.keys()))))
...
def build_physiological_indices(long_df):
    ...
    for _, row in long_df.iterrows():
        ...
        out={
            ...
            'Fase':row.get('Fase',''),
            'Paciente_ID':row.get('Paciente_ID', infer_patient_id(str(row.get('Registro',''))))
        }
        out.update(idxs)
        for _hvg_col in [
...
def physiological_indices_reference_table():
    return pd.DataFrame([
        ...
        {'Índice':'IDX_Regulacion_Lenta','Integra':'VLF, VLF_LS, VLF_DOM_PCT, Hurst y DFAα2 · intensidad, no calidad','0-24':'Muy baja intensidad','25-44':'Baja intensidad','45-59':'Intermedia','60-79':'Alta intensidad','80-100':'Muy alta/dominante'},
    ])
    ...
    try:
        with _cloud_connect_v1540() as con:
            ...
            con.execute(f"""INSERT INTO {CLOUD_TABLE_V1540}
                ...
                (object_key, '15.5.8', sha, len(payload), payload, reason, _instance_id_v1540()))
            ...
            if make_backup:
                ...
                con.execute(f"""INSERT INTO {CLOUD_BACKUP_TABLE_V1540}
                    ...
                    (backup_id, object_key, '15.5.8', sha, len(payload), payload, reason, _instance_id_v1540()))
...
def _cloud_checkpoint_v1540(reason='data_change', min_interval_s=300, force=False):
    ...
    if ok:
        state['last_local_sha'] = local_sha
        # v15.5.20: una escritura PostgreSQL confirmada significa que la imagen
        # remota ya representa el estado local actual. El aviso amarillo debe
        # desaparecer inmediatamente tras una sincronización manual/forzada.
        state['dirty'] = False
        state['dirty_reasons'] = []
        state['last_error'] = ''
    else:
    ...
    for idx, param in enumerate(params):
        ...
        if multi_patient:
            ...
            fig.update_xaxes(
                title_text="Paciente · fecha del registro",
                tickmode="array", tickvals=tick_vals, ticktext=tick_text,
                tickangle=-45, automargin=True, row=rr, col=cc
            )
        else:
            color = _export_color_for(idx)
            if one_phase:
                ...
                fig.update_xaxes(
                    ...
                    tickvals=list(x_num), ticktext=labels, tickangle=-45, automargin=True,
                    row=rr, col=cc
                )
            else:
...
st.caption(f"{APP_VERSION_NOTE} · Atractores animados · Persistencia Cloud · Control autonómico contextual · Nivel 2 XGBoost")
...
with st.sidebar:
    ...
    with st.expander("Historial longitudinal v15.5.8 · Persistent Cloud Architecture II", expanded=True):
        ...
        if _cloud_status_1540['configured']:
            if _cloud_status_1540['connected']:
                ...
                if _cloud_status_1540.get('dirty'):
                    ...
                    st.caption("🟢 Sincronizado con PostgreSQL · no hay cambios locales pendientes. Lazy Sync activo durante la navegación.")
            else:
            ...
            if _cs1.button("Sincronizar histórico con nube", key="cloud_sync_now_v1540"):
                if _cloud_checkpoint_v1540('manual_sync', force=True):
                    st.success("Histórico sincronizado en PostgreSQL V2. No quedan cambios locales pendientes.")
                    st.rerun()
                else:
...
def _configure_attractor_animation(fig, trajectory, dims=3, frame_duration_ms=55, frame_cap=240, marker_name="Punto móvil"):
    """Añade animación Plotly a un atractor ya dibujado sin modificar sus cálculos.

    La trayectoria completa permanece visible y un marcador recorre cronológicamente
    los puntos del embedding. Se limita el número de frames sólo para mantener fluidez
    de renderizado; no se altera el embedding ni los descriptores.
    """
    arr = np.asarray(trajectory, dtype=float)
    if arr.ndim != 2 or len(arr) < 2 or arr.shape[1] < max(2, int(dims)):
        return fig
    nframes = int(min(max(2, frame_cap), len(arr)))
    idx = np.linspace(0, len(arr)-1, nframes).astype(int)
    idx = np.unique(idx)
    if len(idx) < 2:
        return fig
    ...
    if int(dims) >= 3:
        fig.add_trace(go.Scatter3d(
            x=[arr[idx[0],0]], y=[arr[idx[0],1]], z=[arr[idx[0],2]],
            mode="markers", marker=dict(size=7), name=marker_name,
            hovertemplate="Paso %{text}<br>RR(t)=%{x:.1f}<br>RR(t-τ)=%{y:.1f}<br>RR(t-2τ)=%{z:.1f}<extra></extra>",
            text=[str(int(idx[0])+1)]
        ))
        marker_trace = len(fig.data)-1
        frames = [go.Frame(
            data=[go.Scatter3d(
                x=[arr[i,0]], y=[arr[i,1]], z=[arr[i,2]], mode="markers",
                marker=dict(size=7), text=[str(int(i)+1)]
            )], traces=[marker_trace], name=str(k)
        ) for k,i in enumerate(idx)]
    else:
        fig.add_trace(go.Scatter(
            x=[arr[idx[0],0]], y=[arr[idx[0],1]], mode="markers", marker=dict(size=11),
            name=marker_name,
            hovertemplate="Paso %{text}<br>RR(t)=%{x:.1f}<br>RR(t-τ)=%{y:.1f}<extra></extra>",
            text=[str(int(idx[0])+1)]
        ))
        marker_trace = len(fig.data)-1
        frames = [go.Frame(
            data=[go.Scatter(
                x=[arr[i,0]], y=[arr[i,1]], mode="markers", marker=dict(size=11),
                text=[str(int(i)+1)]
            )], traces=[marker_trace], name=str(k)
        ) for k,i in enumerate(idx)]

    fig.frames = frames
    _redraw_anim = bool(int(dims) >= 3)
    steps = [dict(
        method="animate", args=[[str(k)], {"mode":"immediate", "frame":{"duration":0,"redraw":_redraw_anim}, "transition":{"duration":0}}],
        label=str(int(idx[k])+1)
    ) for k in range(len(idx))]
    # Para evitar una etiqueta por cada punto cuando hay muchos frames, Plotly mantiene
    # el slider funcional pero sólo mostramos el valor actual en el prefijo.
    fig.update_layout(
        uirevision="attractor_camera_live_v15514",
        scene=dict(uirevision="attractor_camera_live_v15514") if int(dims) >= 3 else None,
        updatemenus=[dict(
            type="buttons", direction="left", x=0.0, y=0.0, xanchor="left", yanchor="top",
            pad=dict(t=42, r=10), showactive=False,
            buttons=[
                dict(label="▶ Reproducir", method="animate", args=[None, {"fromcurrent":True, "mode":"immediate", "frame":{"duration":int(frame_duration_ms),"redraw":_redraw_anim}, "transition":{"duration":0}}]),
                dict(label="⏸ Pausa", method="animate", args=[[None], {"mode":"immediate", "frame":{"duration":0,"redraw":_redraw_anim}, "transition":{"duration":0}}])
            ]
        )],
        sliders=[dict(
            active=0, x=0.0, y=0.0, len=1.0, xanchor="left", yanchor="top", pad=dict(t=8,b=0),
            currentvalue=dict(prefix="Punto embedding: "), steps=steps
        )],
        margin=dict(b=110)
    )
    return fig
...
def _configure_multi_attractor_animation(fig, tracks, dims=3, frame_duration_ms=55, frame_cap=220):
    """Anima simultáneamente varios atractores superpuestos, cada uno sobre su propia trayectoria."""
    valid=[]
    for name, trajectory in tracks:
        arr=np.asarray(trajectory,dtype=float)
        if arr.ndim==2 and len(arr)>=2 and arr.shape[1]>=max(2,int(dims)):
            valid.append((str(name),arr))
    if not valid:
        return fig
    nframes=int(min(frame_cap, max(len(a) for _,a in valid)))
    nframes=max(2,nframes)
    marker_traces=[]
    idx_maps=[]
    for name,arr in valid:
        imap=np.linspace(0,len(arr)-1,nframes).astype(int)
        idx_maps.append(imap)
        i0=int(imap[0])
        if int(dims)>=3:
            fig.add_trace(go.Scatter3d(x=[arr[i0,0]],y=[arr[i0,1]],z=[arr[i0,2]],mode="markers",marker=dict(size=7),name=f"{name} · punto móvil",showlegend=False))
        else:
            fig.add_trace(go.Scatter(x=[arr[i0,0]],y=[arr[i0,1]],mode="markers",marker=dict(size=10),name=f"{name} · punto móvil",showlegend=False))
        marker_traces.append(len(fig.data)-1)
    frames=[]
    for k in range(nframes):
        data=[]
        for (_,arr),imap in zip(valid,idx_maps):
            i=int(imap[k])
            if int(dims)>=3:
                data.append(go.Scatter3d(x=[arr[i,0]],y=[arr[i,1]],z=[arr[i,2]],mode="markers",marker=dict(size=7)))
            else:
                data.append(go.Scatter(x=[arr[i,0]],y=[arr[i,1]],mode="markers",marker=dict(size=10)))
        frames.append(go.Frame(data=data,traces=marker_traces,name=str(k)))
    fig.frames=frames
    _redraw_anim = bool(int(dims) >= 3)
    fig.update_layout(
        uirevision="attractor_camera_live_v15514",
        scene=dict(uirevision="attractor_camera_live_v15514") if int(dims) >= 3 else None,
        updatemenus=[dict(type="buttons",direction="left",x=0.0,y=0.0,xanchor="left",yanchor="top",pad=dict(t=42,r=10),showactive=False,buttons=[
            dict(label="▶ Reproducir",method="animate",args=[None,{"fromcurrent":True,"mode":"immediate","frame":{"duration":int(frame_duration_ms),"redraw":_redraw_anim},"transition":{"duration":0}}]),
            dict(label="⏸ Pausa",method="animate",args=[[None],{"mode":"immediate","frame":{"duration":0,"redraw":_redraw_anim},"transition":{"duration":0}}])
        ])],
        sliders=[dict(active=0,x=0.0,y=0.0,len=1.0,xanchor="left",yanchor="top",pad=dict(t=8,b=0),currentvalue=dict(prefix="Avance trayectoria: "),steps=[
            dict(method="animate",args=[[str(k)],{"mode":"immediate","frame":{"duration":0,"redraw":_redraw_anim},"transition":{"duration":0}}],label=str(k+1)) for k in range(nframes)
        ])],
        margin=dict(b=110)
    )
    return fig
...
def _export_attractor_animation_bytes(
    tracks, fmt="mp4", fps=20, frame_cap=120,
    elev=24.0, azim=-58.0, zoom=1.0, trail_mode="complete"
):
    """
    v15.5.15 · Exporta atractores 3D a MP4 o GIF.

    - Acepta uno o varios registros.
    - Conserva un color distinto por registro.
    - Cada registro tiene un punto móvil del mismo color.
    - Sincroniza registros de distinta longitud por progreso relativo 0–100%.
    - Cámara configurable (elevación, azimut y zoom).
    - No modifica datos ni cálculos fisiológicos.
    """
    if not VIDEO_ATTRACTOR_AVAILABLE:
        raise RuntimeError("Faltan matplotlib/imageio para generar el vídeo.")

    valid = []
    for name, trajectory in tracks:
        arr = np.asarray(trajectory, dtype=float)
        if arr.ndim == 2 and len(arr) >= 2 and arr.shape[1] >= 3:
            arr = arr[:, :3]
            arr = arr[np.all(np.isfinite(arr), axis=1)]
            if len(arr) >= 2:
                valid.append((str(name), arr))
    if not valid:
        raise ValueError("No hay atractores 3D válidos para exportar.")

    nframes = int(max(12, min(int(frame_cap), max(len(a) for _, a in valid))))
    maps = [np.linspace(0, len(a)-1, nframes).astype(int) for _, a in valid]

    allpts = np.vstack([a for _, a in valid])
    mins = np.nanmin(allpts, axis=0)
    maxs = np.nanmax(allpts, axis=0)
    ctr = (mins + maxs) / 2.0
    span = np.maximum(maxs - mins, 1e-6)
    maxspan = float(np.max(span)) / max(float(zoom), 0.2)
    half = 0.55 * maxspan

    tmpdir = tempfile.mkdtemp(prefix="physiosentinel_attr_video_")
    frame_files = []
    try:
        for k in range(nframes):
            ...
            for j, ((name, arr), imap) in enumerate(zip(valid, maps)):
                c = colors[j % len(colors)]
                idx = int(imap[k])
                if trail_mode == "progressive":
                    line_arr = arr[:idx+1]
                else:
                    line_arr = arr
                ax.plot(line_arr[:,0], line_arr[:,1], line_arr[:,2],
                        linewidth=1.35, color=c, label=name)
                p = arr[idx]
                ax.scatter([p[0]], [p[1]], [p[2]], s=62, color=c,
            ...
            ax.set_xlabel("RR(t)")
            ax.set_ylabel("RR(t-τ)")
            ax.set_zlabel("RR(t-2τ)")
            ax.set_title(f"Atractor 3D · avance {k+1}/{nframes}")
            ax.view_init(elev=float(elev), azim=float(azim))
            ax.legend(loc="upper right", fontsize=8)
            fig.tight_layout()

            fp = Path(tmpdir) / f"frame_{k:04d}.png"
            fig.savefig(fp, bbox_inches="tight")
            plt.close(fig)
            frame_files.append(fp)

        if str(fmt).lower() == "gif":
            out = Path(tmpdir) / "atractor.gif"
            frames = [imageio.imread(str(p)) for p in frame_files]
            imageio.mimsave(str(out), frames, duration=1.0/max(int(fps),1), loop=0)
            mime = "image/gif"
        else:
            out = Path(tmpdir) / "atractor.mp4"
            writer = imageio.get_writer(
                str(out), fps=max(int(fps),1), codec="libx264",
...
def _attractor_video_export_ui(tracks, key_prefix):
    """Controles Streamlit para exportar el/los atractores seleccionados."""
    st.markdown("##### 🎬 Exportar animación del atractor")
    if not VIDEO_ATTRACTOR_AVAILABLE:
        st.warning("La exportación de vídeo necesita matplotlib, imageio e imageio-ffmpeg instalados.")
        return

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        fmt = st.selectbox("Formato", ["MP4", "GIF"], key=f"{key_prefix}_fmt")
    with c2:
        fps = st.slider("FPS", 5, 30, 18, key=f"{key_prefix}_fps")
    with c3:
        nframes = st.select_slider(
            "Frames", options=[30, 60, 90, 120, 180], value=90,
            key=f"{key_prefix}_frames"
        )
    with c4:
        trail_label = st.selectbox(
            "Trayectoria visible",
            ["Completa + punto móvil", "Progresiva"],
            key=f"{key_prefix}_trail"
        )

    st.caption("Cámara del vídeo. Estos controles afectan solo al archivo exportado; no cambian el atractor ni sus cálculos.")
    cam1, cam2, cam3 = st.columns(3)
    with cam1:
        elev = st.slider("Elevación", -80, 80, 24, key=f"{key_prefix}_elev")
    with cam2:
        azim = st.slider("Rotación / azimut", -180, 180, -58, key=f"{key_prefix}_azim")
    with cam3:
        zoom = st.slider("Zoom", 0.5, 2.5, 1.0, 0.1, key=f"{key_prefix}_zoom")

    if st.button("🎬 Generar archivo", key=f"{key_prefix}_generate", use_container_width=True):
        with st.spinner(f"Generando {fmt} del atractor..."):
            try:
                data, mime = _export_attractor_animation_bytes(
                    tracks,
                    fmt=fmt.lower(),
                    fps=int(fps),
                    frame_cap=int(nframes),
                    elev=float(elev),
                    azim=float(azim),
                    zoom=float(zoom),
                    trail_mode="progressive" if trail_label == "Progresiva" else "complete",
                )
                st.session_state[f"{key_prefix}_video_bytes"] = data
                st.session_state[f"{key_prefix}_video_mime"] = mime
                st.session_state[f"{key_prefix}_video_ext"] = fmt.lower()
            except Exception as exc:
                st.error(f"No se pudo generar el vídeo: {exc}")

    data = st.session_state.get(f"{key_prefix}_video_bytes")
    if data:
        ext = st.session_state.get(f"{key_prefix}_video_ext", fmt.lower())
        mime = st.session_state.get(f"{key_prefix}_video_mime", "video/mp4")
        st.download_button(
            f"⬇️ Descargar atractor.{ext}",
            data=data,
            file_name=f"PhysioSentinel_atractor_3D.{ext}",
            mime=mime,
...
def _render_live_attractor_3d(fig, tracks, frame_duration_ms=55, frame_cap=180, height=620):
    """
    v15.5.14 · Animación 3D con interacción real durante PLAY.

    En vez de Plotly frames (que fuerzan redraw de Scatter3d y pueden resetear/bloquear
    la cámara), renderiza el atractor una sola vez y actualiza exclusivamente las
    coordenadas de los marcadores móviles mediante Plotly.restyle() en JavaScript.

    Consecuencia: mientras la trayectoria está reproduciéndose el usuario puede
    rotar, hacer zoom y desplazar la cámara sin que vuelva a la vista inicial.
    """
    valid = []
    for name, trajectory in tracks:
        arr = np.asarray(trajectory, dtype=float)
    ...
    nframes = int(min(max(2, int(frame_cap)), max(len(a) for _, a in valid)))
    nframes = max(2, nframes)

    fig_live = go.Figure(fig)
    marker_indices = []
    js_tracks = []

    _palette_live_v15514 = list(px.colors.qualitative.Plotly) + list(px.colors.qualitative.Safe)
    _base_color_by_name_v15514 = {}
    for _i_base, _tr_base in enumerate(fig_live.data):
        _nm_base = str(getattr(_tr_base, "name", "") or "")
        _c_base = _palette_live_v15514[_i_base % len(_palette_live_v15514)]
        try:
            if getattr(_tr_base, "line", None) is not None and getattr(_tr_base.line, "color", None):
                _c_base = _tr_base.line.color
            else:
                _tr_base.update(line=dict(color=_c_base))
        except Exception:
            pass
        if _nm_base:
            _base_color_by_name_v15514[_nm_base] = _c_base

    for _i_track, (name, arr) in enumerate(valid):
        imap = np.linspace(0, len(arr) - 1, nframes).astype(int)
        coords = arr[imap, :3]
        js_tracks.append(coords.tolist())
        _track_color_v15514 = _base_color_by_name_v15514.get(
            str(name), _palette_live_v15514[_i_track % len(_palette_live_v15514)]
        )
        fig_live.add_trace(go.Scatter3d(
            x=[coords[0, 0]], y=[coords[0, 1]], z=[coords[0, 2]],
            mode="markers",
            marker=dict(size=8, color=_track_color_v15514,
                        line=dict(width=1, color="white")),
            name=f"{name} · punto móvil",
            showlegend=False,
            text=["1"],
            hovertemplate=(
                "<b>" + str(name) + "</b><br>"
                "Paso %{text}<br>"
                "RR(t)=%{x:.1f}<br>RR(t-τ)=%{y:.1f}<br>RR(t-2τ)=%{z:.1f}"
                "<extra></extra>"
            ),
        ))
        marker_indices.append(len(fig_live.data) - 1)

    # No frames. No updatemenus/sliders Plotly. La cámara sólo existe en la escena
    # ya renderizada y nunca se sustituye durante la reproducción.
    fig_live.frames = ()
    ...
    _div_id = "attr3d_" + hashlib.md5(
        ("|".join([n for n, _ in valid]) + str(time.time_ns())).encode("utf-8")
    ).hexdigest()[:14]

    # Plotly genera el gráfico; el JS posterior sólo hace restyle del/los marcador/es.
    _plot_html = fig_live.to_html(
        full_html=False,
    ...
    _tracks_json = json.dumps(js_tracks)
    _trace_json = json.dumps(marker_indices)
    _duration = int(max(15, frame_duration_ms))
    _nframes = int(nframes)

    _controls = f"""
    <div style="display:flex;align-items:center;gap:8px;margin:4px 0 2px 2px;font-family:sans-serif;">
      <button id="{_div_id}_play" style="padding:5px 10px;">▶ Reproducir</button>
      <button id="{_div_id}_pause" style="padding:5px 10px;">⏸ Pausa</button>
      <span id="{_div_id}_label" style="min-width:135px;">Punto embedding: 1</span>
    </div>
    <input id="{_div_id}_slider" type="range" min="0" max="{_nframes-1}" value="0" step="1"
           style="width:99%;margin-left:2px;">
    <script>
    (function() {{
      const gd = document.getElementById("{_div_id}");
      const tracks = {_tracks_json};
      const traceIndices = {_trace_json};
      const nframes = {_nframes};
      const delay = {_duration};
      const slider = document.getElementById("{_div_id}_slider");
      const label = document.getElementById("{_div_id}_label");
      const playBtn = document.getElementById("{_div_id}_play");
      const pauseBtn = document.getElementById("{_div_id}_pause");
      ...
      function drawFrame(i) {{
        ...
        slider.value = i;
        label.textContent = "Punto embedding: " + String(i + 1);
      }}
      ...
      function scheduleNext() {{
        if (timer !== null || interacting) return;
        timer = setTimeout(() => {{
          timer = null;
          if (interacting) return;
          if (k >= nframes - 1) return;
          k += 1;
          drawFrame(k);
          scheduleNext();
        }}, delay);
      ...
      function pointInsidePlot(ev) {{
        const r = gd.getBoundingClientRect();
        return ev.clientX >= r.left && ev.clientX <= r.right &&
               ev.clientY >= r.top  && ev.clientY <= r.bottom;
      }}

      function interactionStart(ev) {{
        if (ev && ev.clientX !== undefined && !pointInsidePlot(ev)) return;
        if (interacting) return;
        interacting = true;
        // Fundamental: durante el gesto NO existe ningún restyle programado.
        // Se pausa internamente y se reanuda automáticamente al soltar.
        stopTimer();
      }}
      ...
      // Captura en document, antes de que WebGL/Plotly pueda consumir el evento.
      document.addEventListener("pointerdown", interactionStart, true);
      document.addEventListener("pointerup", interactionEnd, true);
      document.addEventListener("pointercancel", interactionEnd, true);
      document.addEventListener("mousedown", interactionStart, true);
      document.addEventListener("mouseup", interactionEnd, true);
      document.addEventListener("touchstart", interactionStart, {{passive:true, capture:true}});
      document.addEventListener("touchend", interactionEnd, {{passive:true, capture:true}});

      // Zoom de rueda: detener actualizaciones mientras llegan eventos wheel.
      gd.addEventListener("wheel", (ev) => {{
        interactionStart(ev);
        clearTimeout(wheelEndTimer);
        wheelEndTimer = setTimeout(interactionEnd, 240);
      }}, {{passive:true, capture:true}});

      // Refuerzo específico de Plotly para rotación/pan/zoom de scene.camera.
      if (typeof gd.on === "function") {{
        gd.on("plotly_relayouting", () => {{
          ...
          clearTimeout(relayoutEndTimer);
          relayoutEndTimer = setTimeout(interactionEnd, 220);
        }});
        gd.on("plotly_relayout", () => {{
          clearTimeout(relayoutEndTimer);
          relayoutEndTimer = setTimeout(interactionEnd, 80);
        }});
      }}
      ...
      // PLAY permanece conceptualmente activo durante el gesto: se hace una
      // micro-pausa técnica sólo mientras el usuario mueve la cámara y se
      // reanuda automáticamente al soltar, sin pulsar Pausa/Reproducir.
      drawFrame(0);
    }})();
    </script>
    """
...
# ============================================================
# v15.5.6 · CONTROL AUTONÓMICO MULTIVARIADO · DOBLE PLANO ACTUAL + LONGITUDINAL V/S/L/C/R/Q
...
def _autonomic_abs_from_index(value, center=50.0, scale=20.0):
    """Plano de nivel fisiológico actual a partir de índices 0–100 ya integrados.
    0≈zona media del índice; +1≈20 puntos por encima; -1≈20 por debajo.
    No sustituye el plano longitudinal ni pretende ser un Z poblacional.
    """
    v=_autonomic_num(value)
    if not np.isfinite(v): return np.nan
    return float(np.clip((v-center)/max(scale,1e-9),-2.5,2.5))



def _ensure_patient_reference_profile_table_v15528():
    """Tabla persistente de contexto transversal por paciente."""
    try:
        LONGITUDINAL_DB_PATH.parent.mkdir(parents=True, exist_ok=True)
        with sqlite3.connect(LONGITUDINAL_DB_PATH) as _con:
            _con.execute("""
                CREATE TABLE IF NOT EXISTS patient_reference_profile_v15528 (
...
def _load_patient_reference_profile_v15528(patient_id):
    _ensure_patient_reference_profile_table_v15528()
    _pid = infer_patient_id(patient_id)
    try:
        with sqlite3.connect(LONGITUDINAL_DB_PATH) as _con:
            _row = _con.execute(
                "SELECT age_years, sex, updated_at FROM patient_reference_profile_v15528 WHERE patient_id=?",
                (_pid,)
            ).fetchone()
        if not _row:
            return {"age_years": None, "sex": "No especificado", "updated_at": None}
        return {"age_years": _row[0], "sex": _row[1] or "No especificado", "updated_at": _row[2]}
    except Exception:
        return {"age_years": None, "sex": "No especificado", "updated_at": None}


def _save_patient_reference_profile_v15528(patient_id, age_years, sex):
    _ensure_patient_reference_profile_table_v15528()
    _pid = infer_patient_id(patient_id)
    _age = int(age_years) if age_years is not None else None
    _sex = str(sex or "No especificado")
    _now = datetime.now(timezone.utc).isoformat()
    with sqlite3.connect(LONGITUDINAL_DB_PATH) as _con:
        _con.execute("""
            INSERT INTO patient_reference_profile_v15528(patient_id, age_years, sex, updated_at)
            VALUES(?,?,?,?)
        ...
        _con.commit()
    try:
        _cloud_mark_dirty_v1541("patient_reference_profile_v15528")
    except Exception:
        pass
    return True
...
def _ensure_record_reference_profile_table_v15533():
    """Overrides de edad/sexo por registro concreto."""
    try:
        LONGITUDINAL_DB_PATH.parent.mkdir(parents=True, exist_ok=True)
        with sqlite3.connect(LONGITUDINAL_DB_PATH) as _con:
            _con.execute("""
                CREATE TABLE IF NOT EXISTS record_reference_profile_v15533 (
                    patient_id TEXT NOT NULL,
...
def _load_record_reference_profile_v15533(patient_id, record_id):
    _ensure_record_reference_profile_table_v15533()
    _pid = infer_patient_id(patient_id)
    _rid = str(record_id or "")
    try:
        with sqlite3.connect(LONGITUDINAL_DB_PATH) as _con:
            _row = _con.execute(
                "SELECT age_years, sex, updated_at FROM record_reference_profile_v15533 "
...
def _save_record_reference_profile_v15533(patient_id, record_ids, age_years, sex):
    _ensure_record_reference_profile_table_v15533()
    _pid = infer_patient_id(patient_id)
    _age = int(age_years) if age_years is not None else None
    _sex = str(sex or "No especificado")
    _now = datetime.now(timezone.utc).isoformat()
    with sqlite3.connect(LONGITUDINAL_DB_PATH) as _con:
        for _rid in [str(x) for x in record_ids]:
            _con.execute("""
                INSERT INTO record_reference_profile_v15533(patient_id, record_id, age_years, sex, updated_at)
                VALUES(?,?,?,?,?)
                ON CONFLICT(patient_id, record_id) DO UPDATE SET
                    age_years=excluded.age_years,
                    sex=excluded.sex,
                    updated_at=excluded.updated_at
            """, (_pid, _rid, _age, _sex, _now))
        _con.commit()
    try:
        _cloud_mark_dirty_v1541("record_reference_profile_v15533")
    except Exception:
        pass
    return True


def _resolve_reference_profile_v15533(patient_id, record_id):
    """v15.5.39 · Prioridad override > paciente, usando lectura bulk cacheada."""
    try:
        _maps = _load_all_reference_profiles_v15535(str(LONGITUDINAL_DB_PATH))
        return _resolve_profile_from_maps_v15535(patient_id, record_id, _maps)
    except Exception:
        # fallback seguro hacia la implementación SQL directa
        _rec = _load_record_reference_profile_v15533(patient_id, record_id)
        if _rec is not None:
            return {**_rec, "source": "registro"}
        _pat = _load_patient_reference_profile_v15528(patient_id)
        return {**_pat, "source": "paciente"}


def _age_group_v15528(age_years):
    try:
        a = int(age_years)
    except Exception:
        return None
    if a < 20:
        return "20-39"   # fallback más cercano; se marca como extrapolado
    if a <= 39:
        return "20-39"
    if a <= 59:
        return "40-59"
...
AUTONOMIC_AGE_NORMS_V15528 = {
    "20-39": {
        "RMSSD": (25, 75), "SDNN": (35, 90), "HF": (250, 1200), "LF": (350, 1500),
        "SD1": (18, 53), "SD2": (45, 120), "SampEn": (1.3, 2.1),
        "MeanHR": (60, 78), "pNN50": (8.0, 35.0), "TOTAL": (1500, 4500),
        "VLF": (300, 1200), "LF_HF": (0.8, 2.5),
    },
    "40-59": {
        "RMSSD": (15, 40), "SDNN": (25, 60), "HF": (80, 450), "LF": (150, 600),
        "SD1": (11, 28), "SD2": (30, 80), "SampEn": (1.1, 1.8),
        "MeanHR": (62, 80), "pNN50": (2.0, 15.0), "TOTAL": (600, 2000),
        "VLF": (150, 600), "LF_HF": (1.2, 3.2),
    },
    "60+": {
        "RMSSD": (10, 25), "SDNN": (15, 40), "HF": (30, 180), "LF": (50, 300),
        "SD1": (7, 18), "SD2": (20, 50), "SampEn": (0.8, 1.5),
        "MeanHR": (65, 82), "pNN50": (0.5, 5.0), "TOTAL": (200, 800),
        "VLF": (50, 250), "LF_HF": (1.5, 3.5),
    }
}

AUTONOMIC_STATIC_NORMS_V15528 = {
    "DFA_alpha1": (0.85, 1.15),
    "DFA_alpha2": (0.90, 1.10),
    "Hurst": (0.65, 0.85),
    "D2": (2.5, 4.5),
    "DispEn": (1.8, 2.6),
    "DET": (60.0, 85.0),
...
def _norm_centered_v15528(value, low, high, optimal=False):
    """v15.5.33 · Normalización por rango preservado.

    Regla clave:
    - dentro del rango etario/normativo => 0.0 (preservado)
    - por debajo del límite inferior => negativo proporcional
    - por encima del límite superior => positivo proporcional

    Para métricas con rango óptimo (DFA/Hurst), dentro del rango también = 0;
    fuera del rango se aleja en dirección negativa de organización.
    """
    try:
        v = float(value)
        if not np.isfinite(v) or high <= low:
        ...
        if optimal:
            if v < low:
                return float(np.clip(-(low - v) / width, -1.5, 0.0))
            return float(np.clip(-(v - high) / width, -1.5, 0.0))
        if v < low:
            return float(np.clip(-(low - v) / width, -1.5, 0.0))
        return float(np.clip((v - high) / width, 0.0, 1.5))
    except Exception:
        return np.nan


def _norm_score_0_100_v15528(value, low, high):
    """Puntuación descriptiva contextual por edad: 50 = centro normativo."""
    try:
        v = float(value)
        if not np.isfinite(v) or high <= low:
            return np.nan
        ...
        half = (high-low)/2.0
        z = (v-mid)/half
        return float(np.clip(50.0 + 25.0*z, 0.0, 100.0))
    except Exception:
        return np.nan


def _sex_context_adjustment_v15528(metric, sex):
    """Modulador secundario MUY suave. No altera rangos; sólo trazabilidad contextual."""
    s = str(sex or "").strip().lower()
    if s.startswith("muj"):
        if metric == "MeanHR": return +0.10
        if metric in ("RMSSD","pNN50","HF"): return +0.05
        if metric in ("TOTAL","LF"): return -0.05
    if s.startswith("hom") or s.startswith("var"):
        if metric in ("TOTAL","LF","VLF"): return +0.05
        if metric == "MeanHR": return -0.05
    return 0.0


def _autonomic_age_context_scores_v15528(row, age_years=None, sex="No especificado"):
    """Normalización transversal contextual. Nunca usa histórico."""
    grp = _age_group_v15528(age_years)
    if grp is None:
        return {"group": None, "scores": {}, "details": [], "age": age_years, "sex": sex}

    norms = AUTONOMIC_AGE_NORMS_V15528[grp]
    scores, details = {}, []
    for metric, (lo, hi) in norms.items():
        val = _autonomic_num(row.get(metric, np.nan))
        if not np.isfinite(val):
            continue
        sc = _norm_centered_v15528(val, lo, hi)
        sc += _sex_context_adjustment_v15528(metric, sex)
        sc = float(np.clip(sc, -1.5, 1.5))
        scores[metric] = sc
        pos = "dentro" if lo <= val <= hi else ("por debajo" if val < lo else "por encima")
        details.append({
            "Parámetro": metric, "Valor": val, "Rango_edad": f"{lo} – {hi}",
            "Posición": pos, "Score_contextual": sc
        })

    # Rangos no estratificados por edad, usados sólo para C/L/rigidez.
    for metric, (lo, hi) in AUTONOMIC_STATIC_NORMS_V15528.items():
        val = _autonomic_num(row.get(metric, np.nan))
        if np.isfinite(val):
            if metric in ("DFA_alpha1","DFA_alpha2","Hurst"):
                sc = _norm_centered_v15528(val, lo, hi, optimal=True)
            elif metric == "DET":
                # alto DET = más rigidez, por tanto para complejidad se usa inverso después
                sc = _norm_centered_v15528(val, lo, hi)
            else:
                sc = _norm_centered_v15528(val, lo, hi)
            scores[metric] = sc
    ...
    return {"group": grp, "scores": scores, "details": details, "age": age_years, "sex": sex}


def _autonomic_abs_vector_levels_v15542(row, age_years=None, sex="No especificado"):
    """v15.5.42 · V/S/L/C/R actuales, 100% transversales.

    Mejoras de S (Alerta/Barorreflejo; NO simpático puro):
    - LF absoluto se atenúa cuando existe evidencia vagal/HF fuerte.
    - Se añade geometría Poincaré SD1/SD2 como evidencia ortogonal al espectro.
    - No utiliza ningún dato longitudinal.
    """
    ctx = _autonomic_age_context_scores_v15528(row, age_years=age_years, sex=sex)
    s = ctx["scores"]
    get = lambda k: _autonomic_num(row.get(k, np.nan))

    # --------------------------- V ---------------------------
    V = _autonomic_weighted_score([
        (s.get("RMSSD", np.nan), 1.00),
        (s.get("SD1", np.nan), 1.00),
        (s.get("HF", np.nan), 0.90),
        (s.get("pNN50", np.nan), 0.55),
        (_norm_centered_v15528(get('HF_DOM_PCT'), 10, 65), 0.55),
    ])

    # --------------------------- S ---------------------------
    # LF no se interpreta como simpático puro. Su peso se reduce cuando HF es
    # claramente dominante/elevada, porque la banda LF puede contener modulación vagal/barorrefleja.
    lfdom, hfdom = get('LF_DOM_PCT'), get('HF_DOM_PCT')
    rel_lf = np.nan
    if np.isfinite(lfdom) and np.isfinite(hfdom):
        rel_lf = float(np.clip((lfdom-hfdom)/25.0, -1.5, 1.5))

    hf_ctx = s.get("HF", np.nan)
    hf_strength = float(np.clip(max(hf_ctx, 0.0), 0.0, 1.5)) if np.isfinite(hf_ctx) else 0.0
    hf_dom_strength = 0.0
    if np.isfinite(hfdom) and np.isfinite(lfdom):
        hf_dom_strength = float(np.clip((hfdom-lfdom)/35.0, 0.0, 1.0))
    lf_vagal_attenuation = float(np.clip(1.0 - 0.30*hf_strength - 0.30*hf_dom_strength, 0.35, 1.0))

    lf_term = s.get("LF", np.nan)
    if np.isfinite(lf_term):
        lf_term = float(lf_term) * lf_vagal_attenuation

    lfhf_term = s.get("LF_HF", np.nan)
    if np.isfinite(lfhf_term):
        # LF/HF sólo apoyo secundario; también se amortigua modestamente con HF fuerte.
        lfhf_term = float(lfhf_term) * float(np.clip(0.70 + 0.30*lf_vagal_attenuation, 0.70, 1.0))

    # Geometría Poincaré: SD1/SD2 bajo = poca dispersión rápida respecto a la lenta,
    # compatible con mayor peso de regulación alerta/barorrefleja. Ratio alto reduce S.
    sd1, sd2 = get('SD1'), get('SD2')
    poincare_geometry = np.nan
    if np.isfinite(sd1) and np.isfinite(sd2) and sd1 > 0 and sd2 > 0:
        sd1_sd2 = float(sd1/sd2)
        poincare_geometry = float(np.clip((0.45 - sd1_sd2)/0.25, -1.0, 1.0))

    # Asimetría opcional si en futuras exportaciones ya existen SD1d/SD1a.
    # No se fuerza si no está disponible.
    sd1d, sd1a = get('SD1d'), get('SD1a')
    poincare_asym = np.nan
    if np.isfinite(sd1d) and np.isfinite(sd1a) and (sd1d + sd1a) > 0:
        # Se usa como modulador pequeño, no como evidencia simpática directa.
        poincare_asym = float(np.clip((sd1a - sd1d)/(sd1a + sd1d), -1.0, 1.0))

    S = _autonomic_weighted_score([
        (lf_term, 0.30),
        (lfhf_term, 0.25),
        (s.get("MeanHR", np.nan), 0.20),
        (s.get("DFA_alpha1", np.nan), 0.20),
        (rel_lf, 0.80),
        (poincare_geometry, 0.35),
        (poincare_asym, 0.12),
    ])

    # --------------------------- L ---------------------------
    # Intensidad/desviación lenta, no escala de calidad.
    L = _autonomic_weighted_score([
        (s.get("VLF", np.nan), 1.00),
        (s.get("DFA_alpha2", np.nan), 0.45),
        (s.get("Hurst", np.nan), 0.35),
    ])

    # --------------------------- C ---------------------------
    det_c = -s.get("DET", np.nan) if np.isfinite(s.get("DET", np.nan)) else np.nan
    C = _autonomic_weighted_score([
        (s.get("SampEn", np.nan), 0.85),
        (s.get("DispEn", np.nan), 0.65),
        (s.get("D2", np.nan), 0.60),
        (det_c, 0.35),
        (s.get("DFA_alpha1", np.nan), 0.25),
    ])

    # --------------------------- R ---------------------------
    R = _autonomic_weighted_score([
        (s.get("SDNN", np.nan), 1.00),
        (s.get("SD2", np.nan), 0.90),
        (s.get("TOTAL", np.nan), 0.80),
    ])

    ctx = dict(ctx)
    ctx['S_refinement_v15542'] = {
        'LF_vagal_attenuation': lf_vagal_attenuation,
        'Poincare_SD1_SD2_support': poincare_geometry,
        'Poincare_asymmetry_support': poincare_asym,
    }
    return {'V':V,'S':S,'L':L,'C':C,'R':R,'context':ctx}


def _reserve_context_label_v15528(R_abs, ctx):
    if not np.isfinite(R_abs):
        return "Amplitud no determinada"
    # Etiqueta contextual, no universal.
    if R_abs >= 0.75:
        return "Amplitud Alta para su grupo de edad"
    if R_abs >= -0.25:
        return "Amplitud Dentro de rango para su grupo de edad"
    if R_abs >= -0.75:
        return "Amplitud Baja-intermedia para su grupo de edad"
    return "Amplitud Baja para su grupo de edad"


def _autonomic_abs_vector_levels(row):
    """v15.5.23 · Nivel actual V/S/L/C/R sin usar información longitudinal.

    Cambio exclusivo:
    - S_actual deja de derivar de IDX_Rigidez.
    - IDX_Rigidez se conserva como descriptor independiente de organización/rigidez.
    - S_actual se construye con evidencia transversal más específica de alerta/barorreflejo:
      LF_DOM_PCT, LF/HF (apoyo secundario), DFA_alpha1, MeanHR y LF absoluto.
    """
    get = lambda k: _autonomic_num(row.get(k, np.nan))

    # V, R, C y L conservan exactamente la arquitectura previa.
    V = _autonomic_abs_from_index(get('IDX_Vagal'))
    R = _autonomic_abs_from_index(get('IDX_Amplitud'))
    C = _autonomic_abs_from_index(get('IDX_Complejidad'))
    L = _autonomic_abs_from_index(get('IDX_Regulacion_Lenta'))

    # ---------------------------------------------------------------
    # NUEVO S_actual: evidencia transversal específica.
    # ---------------------------------------------------------------
    lfdom = get('LF_DOM_PCT')
    hfdom = get('HF_DOM_PCT')
    lfhf = get('LF_HF')
    dfa1 = get('DFA_alpha1')
    meanhr = get('MeanHR')
    lf_abs = get('LF')
    ...
    # Dominancia LF relativa frente a HF: señal principal de componente
    # barorreflejo relativo, sin equipararla a simpático puro.
    if np.isfinite(lfdom) and np.isfinite(hfdom):
        rel_baroref = float(np.clip((lfdom - hfdom) / 25.0, -1.5, 1.5))
        s_terms.append((rel_baroref, 1.00))
    elif np.isfinite(lfdom):
        rel_baroref = float(np.clip((lfdom - 33.0) / 20.0, -1.25, 1.25))
        s_terms.append((rel_baroref, 0.65))

    # LF/HF: sólo apoyo secundario, nunca como "simpático puro".
    if np.isfinite(lfhf):
        try:
            lfhf_support = float(np.clip(np.log(max(lfhf, 1e-6) / 1.5), -1.25, 1.25))
            s_terms.append((lfhf_support, 0.30))
        except Exception:
            pass

    # DFA alpha1: organización rápida secundaria.
    if np.isfinite(dfa1) and dfa1 >= 0.5:
        dfa_alert = float(np.clip((dfa1 - 1.0) / 0.30, -1.0, 1.5))
        s_terms.append((dfa_alert, 0.30))

    # Frecuencia cardiaca: contexto pequeño, no decisivo.
    if np.isfinite(meanhr):
        hr_support = float(np.clip((meanhr - 70.0) / 20.0, -1.0, 1.0))
        s_terms.append((hr_support, 0.20))

    # Potencia LF absoluta: apoyo moderado, combinando Welch/Lomb si existen.
    lf_abs_terms = []
    if np.isfinite(lf_abs) and lf_abs > 0:
        lf_abs_terms.append(np.log10(lf_abs))
    if np.isfinite(lf_ls) and lf_ls > 0:
        lf_abs_terms.append(np.log10(lf_ls))
    if lf_abs_terms:
        lf_log = float(np.mean(lf_abs_terms))
        lf_support = float(np.clip((lf_log - 2.0) / 1.0, -1.0, 1.0))
        s_terms.append((lf_support, 0.25))

    S = _autonomic_weighted_score(s_terms) if s_terms else np.nan

    # ---------------------------------------------------------------
    # V actual conserva IDX_Vagal como base y sólo añade dominancia relativa.
    # ---------------------------------------------------------------
    if np.isfinite(hfdom) and np.isfinite(lfdom):
        vagal_dom = float(np.clip((hfdom - lfdom) / 25.0, -1.5, 1.5))
        V = _autonomic_weighted_score([(V, 0.80), (vagal_dom, 0.20)])

    return {'V': V, 'S': S, 'L': L, 'C': C, 'R': R}

def _autonomic_fuse_planes(abs_level, long_level, abs_w=0.70, long_w=0.30):
    """Combina nivel actual + cambio longitudinal sin dejar que un Z≈0 borre un nivel alto."""
    if np.isfinite(abs_level) and np.isfinite(long_level):
        return float(abs_w*abs_level + long_w*long_level)
    if np.isfinite(abs_level): return float(abs_level)
    if np.isfinite(long_level): return float(long_level)
    return np.nan


def _autonomic_reserve_label_v15525(R_abs):
    """Reserva transversal ABSOLUTA derivada de IDX_Amplitud actual.

    R_abs = (IDX_Amplitud - 50) / 20.
    Los cortes se hacen equivalentes a la escala clínica visible del índice:
    <25 muy baja, 25-44 baja, 45-59 intermedia, 60-79 alta, >=80 muy alta.
    La posición respecto a sesiones previas se informa aparte y nunca cambia esta etiqueta.
    """
    if not np.isfinite(R_abs):
        return "Reserva no determinada"
    idx_amp = float(np.clip(50.0 + 20.0 * R_abs, 0.0, 100.0))
    if idx_amp < 25:
        return "Reserva Muy Baja"
    if idx_amp < 45:
        return "Reserva Baja"
    if idx_amp < 60:
        return "Reserva Intermedia"
    if idx_amp < 80:
        return "Reserva Alta"
    return "Reserva Muy Alta"


def _autonomic_reserve_relative_label_v15525(R_long):
    """Cambio intrapersonal de reserva; separado del nivel absoluto actual."""
    if not np.isfinite(R_long):
        return "Referencia insuficiente"
    if R_long >= 1.50:
        return "Ganancia marcada de reserva respecto al histórico"
    if R_long >= 0.80:
        return "Ganancia moderada de reserva respecto al histórico"
    if R_long <= -1.50:
        return "Pérdida marcada de reserva respecto al histórico"
    if R_long <= -0.80:
        return "Pérdida moderada de reserva respecto al histórico"
    return "Reserva longitudinal estable / cambio leve"


def _autonomic_classify_instantaneous_v15542(
    V_abs, S_abs, L_abs, C_abs, R_abs,
    hfdom=np.nan, lfdom=np.nan, lfhf=np.nan,
    idx_amp=np.nan, oscillatory_current=None, context_scores=None,
    quality=0.75
):
    """v15.5.42 · Topología instantánea fuzzy/adaptativa.

    - Estado actual 100% transversal.
    - Zona gris depende sólo de calidad del registro actual (N, duración, artefactos...), nunca del histórico.
    - Desacoplamiento fisiológico unilateral se separa de incoherencia metodológica/señal.
    - Las pertenencias son continuas para reducir saltos por cortes rígidos.
    """
    oscillatory_current = oscillatory_current or {}
    context_scores = context_scores or {}
    osc_pat = str(oscillatory_current.get('patron', ''))
    osc_note = str(oscillatory_current.get('nota', ''))
    reserve = _reserve_context_label_v15528(R_abs, None)

    q = float(np.clip(quality if np.isfinite(quality) else 0.75, 0.20, 1.0))
    # Mejor calidad -> zona gris más estrecha. Peor calidad -> más prudencia.
    gray = float(np.clip(0.14 + (1.0-q)*0.14, 0.14, 0.25))
    transition_width = float(np.clip(0.10 + (1.0-q)*0.08, 0.10, 0.17))
    ...
    def _memberships(x):
        if not np.isfinite(x):
            return {'low':0.0,'preserved':0.0,'high':0.0}
        high = _sigmoid((x-gray)/transition_width)
        low = _sigmoid((-x-gray)/transition_width)
        preserved = float(np.clip(1.0-max(high,low),0.0,1.0))
        # Normalizar para interpretación probabilística.
        total = low+preserved+high
        if total <= 0:
            return {'low':0.0,'preserved':1.0,'high':0.0}
        return {'low':low/total,'preserved':preserved/total,'high':high/total}

    def _state(x):
        m=_memberships(x)
        if not np.isfinite(x): return 'N/D'
        label=max(m.items(),key=lambda kv:kv[1])[0]
        return {'low':'Reducido','preserved':'Preservado','high':'Elevado'}[label]

    vals=[V_abs,S_abs,L_abs,C_abs,R_abs]
    if sum(np.isfinite(x) for x in vals)<3 or not(np.isfinite(V_abs) and np.isfinite(S_abs)):
        return {
            'topologia':'Patrón intermedio/no concluyente',
            'calificador_topologico':'información insuficiente',
            'topologia_mostrada':'Patrón intermedio/no concluyente · información insuficiente',
            'predominio':'No determinado','reserva':reserve,
            'estado':'Patrón intermedio/no concluyente · información insuficiente',
            'predominio_vectorial':'No determinado','fuente_predominio':'No determinada',
            'explicacion_predominio':'Información insuficiente para establecer predominio.',
            'apoyo_espectral':'Evidencia insuficiente','discordancia_espectral':False,
            'HF_DOM_actual':hfdom,'LF_DOM_actual':lfdom,'LF_HF_actual':lfhf,
            'Amplitud_absoluta_idx':idx_amp,'Patron_oscilatorio_actual_apoyo':osc_pat,
            'Nota_convergencia_actual':'No hay suficientes dimensiones actuales.',
            'Score_Coactivacion':np.nan,'Score_Coactivacion_bruto':np.nan,
            'Score_Reciprocidad':np.nan,'Score_Reciprocidad_bruto':np.nan,
            'Score_Coinhibicion':np.nan,'Score_Coinhibicion_bruto':np.nan,
            'Score_Desacoplamiento':np.nan,'Score_Desacoplamiento_bruto':np.nan,
            'Score_Incoherencia':np.nan,'Score_Intermedio':np.nan,
            'Desacoplamiento_permitido':False,'Incoherencia_detectada':False,
            'Coactivacion_permitida':False,'Reciprocidad_permitida':False,'Coinhibicion_permitida':False,
            'Estado_V_actual':_state(V_abs),'Estado_S_actual':_state(S_abs),
            'Pertenencia_V':_memberships(V_abs),'Pertenencia_S':_memberships(S_abs),
            'Umbral_gris_actual':gray,'Contribucion_VS_pct':70,'Contribucion_LCR_pct':30,
            'Equilibrio_Lento_actual':np.nan,'Carga_Lenta_elevada_actual':False,
            'Expresion_Lenta_reducida_actual':False,'Actividad_multibanda_actual':False,
            'Motivo_compuerta_coinhibicion':'Información insuficiente'
        }

    # -------------------- Espectro actual --------------------
    hf_gt_lf=np.isfinite(hfdom) and np.isfinite(lfdom) and hfdom>lfdom
    lf_gt_hf=np.isfinite(hfdom) and np.isfinite(lfdom) and lfdom>hfdom
    spectral_delta=float(hfdom-lfdom) if np.isfinite(hfdom) and np.isfinite(lfdom) else np.nan
    vagal_spec=bool(hf_gt_lf or (np.isfinite(lfhf) and lfhf<1.5))
    alert_spec=bool(lf_gt_hf and ((np.isfinite(lfhf) and lfhf>1.5) or (np.isfinite(lfdom) and lfdom>40)))
    strong_vagal_spec=bool(vagal_spec and ((np.isfinite(spectral_delta) and spectral_delta>=10) or (np.isfinite(hfdom) and hfdom>=45 and hf_gt_lf)))
    strong_alert_spec=bool(alert_spec and ((np.isfinite(spectral_delta) and spectral_delta<=-10) or (np.isfinite(lfdom) and lfdom>=45 and lf_gt_hf)))
    spectral_biactive=bool(np.isfinite(hfdom) and np.isfinite(lfdom) and hfdom>=20 and lfdom>=20 and abs(hfdom-lfdom)<=25)

    # -------------------- Multibanda --------------------
    band_scores={k:context_scores.get(k,np.nan) for k in ('HF','LF','VLF','TOTAL')}
    finite_bands=[float(v) for v in band_scores.values() if np.isfinite(v)]
    n_not_reduced=sum(v>=-0.25 for v in finite_bands)
    n_preserved=sum(v>=0.0 for v in finite_bands)
    total_ctx=band_scores.get('TOTAL',np.nan)
    multiband_current=bool(len(finite_bands)>=3 and n_not_reduced>=2 and (not np.isfinite(total_ctx) or total_ctx>=-0.50))
    multiband_strong=bool(len(finite_bands)>=3 and n_preserved>=2 and (not np.isfinite(total_ctx) or total_ctx>=-0.25))

    # -------------------- Fuzzy V/S --------------------
    mv,ms=_memberships(V_abs),_memberships(S_abs)
    vector_gap=float(V_abs-S_abs); abs_gap=abs(vector_gap)

    l_balance=float(np.clip(1.0-abs(L_abs),0,1)) if np.isfinite(L_abs) else 0.5
    l_low=float(np.clip(-L_abs,0,1)) if np.isfinite(L_abs) else 0.0
    l_high=float(np.clip(L_abs,0,1)) if np.isfinite(L_abs) else 0.0
    mc=_memberships(C_abs); mr=_memberships(R_abs)
    c_function=float(mc['preserved']+0.65*mc['high'])
    r_function=float(mr['preserved']+0.65*mr['high'])
    lcr_functional=float(np.clip(0.25*l_balance+0.40*c_function+0.35*r_function,0,1))
    lcr_reduced=float(np.clip(0.25*l_low+0.35*mc['low']+0.40*mr['low'],0,1))

    # -------------------- Incoherencia metodológica/señal --------------------
    arr=np.array([x for x in vals if np.isfinite(x)],dtype=float)
    dispersion=float(np.clip(np.std(arr)/0.85,0,1))
    signs=np.sign(arr[np.abs(arr)>gray]) if len(arr) else np.array([])
    sign_conflict=float(min(np.mean(signs>0),np.mean(signs<0))*2.0) if len(signs)>=2 else 0.0

    explicit_spectral_conflict=bool((vector_gap>=0.35 and strong_alert_spec) or (vector_gap<=-0.35 and strong_vagal_spec))
    rc_conflict=bool(np.isfinite(R_abs) and np.isfinite(C_abs) and ((R_abs<-0.65 and C_abs>0.45) or (R_abs>0.45 and C_abs<-0.65)))
    multidim_conflict=bool(dispersion>=0.70 and sign_conflict>=0.55)
    technical_discordance=bool(explicit_spectral_conflict or rc_conflict or multidim_conflict)

    # -------------------- Desacoplamiento fisiológico unilateral --------------------
    # Una rama claramente desplazada mientras la otra tiene alta pertenencia preservada/neutra.
    v_shift=max(mv['high'],mv['low'])
    s_shift=max(ms['high'],ms['low'])
    vagal_unilateral=bool(v_shift>=0.62 and ms['preserved']>=0.60 and abs(S_abs)<=max(0.10,gray*0.70))
    alert_unilateral=bool(s_shift>=0.62 and mv['preserved']>=0.60 and abs(V_abs)<=max(0.10,gray*0.70))
    unilateral_decoupling=bool((vagal_unilateral or alert_unilateral) and not technical_discordance)

    # -------------------- Familia fisiológica --------------------
    joint_high=float(min(mv['high'],ms['high']))
    joint_pres=float(min(mv['preserved'],ms['preserved']))
    joint_low=float(min(mv['low'],ms['low']))
    separation=float(np.clip((abs_gap-gray)/(0.70-gray),0,1)) if gray<0.70 else 0.0

    coact_supports=[multiband_current,spectral_biactive,lcr_functional>=0.68,(c_function>=0.62 and r_function>=0.62)]
    n_coact_supports=int(sum(bool(x) for x in coact_supports))
    coact_core=float(np.clip(max(joint_high, joint_pres*(n_coact_supports/4.0)),0,1))
    coactivation_allowed=bool((joint_high>=0.38) or (joint_pres>=0.50 and n_coact_supports>=2 and abs_gap<=gray*1.25))

    # Reciprocidad: oposición / separación direccional; evita confundir unilateral neutro con reciprocidad.
    reciprocal_opposition=max(min(mv['high'],ms['low']),min(ms['high'],mv['low']))
    reciprocal_direction=max(min(mv['high'],ms['preserved']),min(ms['high'],mv['preserved']))
    spectral_align=1.0 if ((vector_gap>0 and vagal_spec) or (vector_gap<0 and alert_spec)) else 0.0
    recip_core=float(np.clip(0.45*separation+0.35*reciprocal_opposition+0.20*max(reciprocal_direction,spectral_align*0.65),0,1))
    reciprocity_allowed=bool(not unilateral_decoupling and not technical_discordance and (reciprocal_opposition>=0.28 or (separation>=0.32 and abs_gap>gray*1.35)))

    coinhib_core=float(np.clip(0.75*joint_low+0.25*lcr_reduced,0,1))
    coinhibition_allowed=bool(joint_low>=0.42 and not multiband_strong and not technical_discordance)

    uncoupled_core=float(np.clip(max(v_shift*ms['preserved'],s_shift*mv['preserved']),0,1))
    score_uncoupled_raw=float(0.75*uncoupled_core+0.25*lcr_functional)

    incoherence_raw=float(np.clip(0.40*dispersion+0.25*sign_conflict+0.20*(1.0 if explicit_spectral_conflict else 0.0)+0.15*(1.0 if rc_conflict else 0.0),0,1))

    score_coact_raw=float(0.70*coact_core+0.30*lcr_functional)
    score_recip_raw=float(0.70*recip_core+0.30*lcr_functional)
    score_coinhib_raw=float(0.70*coinhib_core+0.30*lcr_reduced)

    score_coact=score_coact_raw if coactivation_allowed and not technical_discordance else -1.0
    score_recip=score_recip_raw if reciprocity_allowed else -1.0
    score_coinhib=score_coinhib_raw if coinhibition_allowed else -1.0
    score_uncoupled=score_uncoupled_raw if unilateral_decoupling else -1.0
    score_incoherence=incoherence_raw if technical_discordance else -1.0

    max_phys=max([v for v in [score_coact,score_recip,score_coinhib,score_uncoupled] if v>=0] or [0.0])
    ambiguity=float(np.clip(1.0-max_phys,0,1))
    score_intermediate=float(np.clip(0.55*ambiguity+0.45*min(mv['preserved'],ms['preserved'])*(1.0 if n_coact_supports<2 else 0.35),0,1))
    if technical_discordance:
        score_intermediate=float(np.clip(score_intermediate+0.20,0,1))

    scores={
        'Coactivación':score_coact,
        'Reciprocidad':score_recip,
        'Coinhibición':score_coinhib,
        'Desacoplamiento fisiológico unilateral':score_uncoupled,
        'Patrón complejo/incoherente':score_incoherence,
        'Patrón intermedio/no concluyente':score_intermediate,
    }
    ordered=sorted(scores.items(),key=lambda kv:kv[1],reverse=True)
    topology,best=ordered[0]
    second=ordered[1][1]

    # Evitar asignaciones fisiológicas débiles; incoherencia sí puede dominar directamente.
    if topology not in ('Patrón complejo/incoherente','Patrón intermedio/no concluyente') and best<0.38:
        topology='Patrón intermedio/no concluyente'; best=score_intermediate
    margin=best-second
    if topology=='Patrón complejo/incoherente': qualifier='requiere revisión de señal/métodos'
    elif topology=='Patrón intermedio/no concluyente': qualifier='sin evidencia suficiente para una familia única'
    elif margin<0.07: qualifier='limítrofe'
    elif margin<0.14: qualifier='moderadamente definida'
    else: qualifier='definida'
    if topology=='Coactivación' and lcr_functional>=0.65: qualifier='funcional por convergencia multidimensional'

    # -------------------- Predominio: jerarquía v15.5.38 con zona adaptativa --------------------
    pred_thr=max(0.20,gray)
    if vector_gap>=pred_thr:
        vector_pred=predominance='Predominio Vagal'; predominance_source='V/S'
    elif vector_gap<=-pred_thr:
        vector_pred=predominance='Predominio Alerta/Barorreflejo'; predominance_source='V/S'
    else:
        vector_pred=predominance='Sin predominio claro'; predominance_source='V/S próximo'

    spectral_note='Sin evidencia espectral transversal concluyente'; discordance=False
    if vagal_spec and not alert_spec:
        spectral_note='Espectro actual apoya componente vagal'
        if abs_gap<pred_thr:
            predominance='Predominio Vagal'; predominance_source='Espectro con V/S próximo'
        elif vector_gap<=-pred_thr:
            discordance=True; spectral_note='Discordancia: espectro vagal frente a S>V; se conserva V/S.'
    elif alert_spec and not vagal_spec:
        spectral_note='Espectro actual apoya componente alerta/barorreflejo'
        if abs_gap<pred_thr:
            predominance='Predominio Alerta/Barorreflejo'; predominance_source='Espectro con V/S próximo'
        elif vector_gap>=pred_thr:
            discordance=True; spectral_note='Discordancia: espectro alerta/barorreflejo frente a V>S; se conserva V/S.'
    elif vagal_spec and alert_spec:
        spectral_note='Espectro actual mixto/no concluyente'

    predominance_explanation=(f'V={V_abs:+.2f}, S={S_abs:+.2f}, V−S={vector_gap:+.2f}. '
        f'Zona gris adaptativa ±{gray:.2f} (calidad actual={q:.2f}). Fuente: {predominance_source}. {spectral_note}')

    topology_display=topology if qualifier=='definida' else f'{topology} · {qualifier}'
    state=f'{topology_display} / {predominance} ({reserve})'
    gate_note=(f'Fuzzy Q={q:.2f}, gris=±{gray:.2f}. '
        f'Coactivación={"sí" if coactivation_allowed else "no"} ({n_coact_supports}/4 apoyos); '
        f'Reciprocidad={"sí" if reciprocity_allowed else "no"}; Coinhibición={"sí" if coinhibition_allowed else "no"}; '
        f'Desacoplamiento unilateral={"sí" if unilateral_decoupling else "no"}; '
        f'Incoherencia={"sí" if technical_discordance else "no"}.')

    return {
        'topologia':topology,'calificador_topologico':qualifier,'topologia_mostrada':topology_display,
        'predominio':predominance,'reserva':reserve,'estado':state,'predominio_vectorial':vector_pred,
        'fuente_predominio':predominance_source,'explicacion_predominio':predominance_explanation,
        'apoyo_espectral':spectral_note,'discordancia_espectral':bool(discordance),
        'HF_DOM_actual':hfdom,'LF_DOM_actual':lfdom,'LF_HF_actual':lfhf,'Amplitud_absoluta_idx':idx_amp,
        'Patron_oscilatorio_actual_apoyo':osc_pat,
        'Nota_convergencia_actual':('Topología fuzzy/adaptativa. Desacoplamiento fisiológico se separa de incoherencia de señal. '
            f'Scores → Coact {score_coact:.2f}, Recip {score_recip:.2f}, Coinhib {score_coinhib:.2f}, '
            f'Desacoplamiento {score_uncoupled:.2f}, Incoherencia {score_incoherence:.2f}, Intermedio {score_intermediate:.2f}. {gate_note} {osc_note}'),
        'Score_Coactivacion':float(score_coact),'Score_Coactivacion_bruto':float(score_coact_raw),
        'Score_Reciprocidad':float(score_recip),'Score_Reciprocidad_bruto':float(score_recip_raw),
        'Score_Coinhibicion':float(score_coinhib),'Score_Coinhibicion_bruto':float(score_coinhib_raw),
        'Score_Desacoplamiento':float(score_uncoupled),'Score_Desacoplamiento_bruto':float(score_uncoupled_raw),
        'Score_Incoherencia':float(score_incoherence),'Score_Intermedio':float(score_intermediate),
        'Desacoplamiento_permitido':bool(unilateral_decoupling),'Incoherencia_detectada':bool(technical_discordance),
        'Coactivacion_permitida':bool(coactivation_allowed),'Reciprocidad_permitida':bool(reciprocity_allowed),
        'Coinhibicion_permitida':bool(coinhibition_allowed),'Estado_V_actual':_state(V_abs),'Estado_S_actual':_state(S_abs),
        'Pertenencia_V':mv,'Pertenencia_S':ms,'Umbral_gris_actual':gray,
        'Contribucion_VS_pct':70,'Contribucion_LCR_pct':30,
        'Equilibrio_Lento_actual':float(l_balance),'Carga_Lenta_elevada_actual':bool(l_high>=0.50),
        'Expresion_Lenta_reducida_actual':bool(l_low>=0.50),'Actividad_multibanda_actual':bool(multiband_current),
        'Actividad_multibanda_actual_fuerte':bool(multiband_strong),'Motivo_compuerta_coinhibicion':gate_note,
        'Band_scores_contextuales':band_scores,'Apoyos_coactivacion_preservada':int(n_coact_supports),
    }


def _autonomic_classify_integrated_v15527(V_eff, S_eff, L_eff, C_eff, R_eff):
    """Plano integrado 70/30.

    Es una síntesis secundaria:
    70% estado actual + 30% longitudinal.
    No redefine ni el estado actual ni el longitudinal.
    """
    if not (np.isfinite(V_eff) and np.isfinite(S_eff)):
        topology = "Integrado no determinado"
        predominance = "No determinado"
    else:
        if V_eff >= 0 and S_eff >= 0:
            topology = "Coactivación integrada"
        elif V_eff < 0 and S_eff < 0:
            topology = "Coinhibición integrada"
        else:
            topology = "Reciprocidad integrada"

        if V_eff >= S_eff + 0.20:
            predominance = "Predominio Vagal"
        elif S_eff >= V_eff + 0.20:
            predominance = "Predominio Alerta/Barorreflejo"
        else:
            predominance = "Equilibrado"

    if np.isfinite(R_eff):
        reserve = (
            "Reserva integrada alta" if R_eff >= 0.75 else
            "Reserva integrada intermedia" if R_eff >= -0.25 else
            "Reserva integrada reducida"
        )
    else:
        reserve = "Reserva integrada no determinada"

    return {
        "estado_integrado": f"{topology} / {predominance} ({reserve})",
        "topologia_integrada": topology,
        "predominio_integrado": predominance,
        "reserva_integrada": reserve,
    }


def _autonomic_longitudinal_severity_v15522(values, trend):
    """Severidad por magnitud máxima de desplazamientos disponibles."""
    vals = [abs(float(v)) for v in values if np.isfinite(v)]
    if not vals or trend in ("Estabilidad Longitudinal", "Referencia longitudinal insuficiente"):
...
def _autonomic_classify_longitudinal_core_v15522(
    ref_n, total_z, lf_z, mse_z, mde_z, c_z, r_z, v_z, s_z, l_z,
    entropy_z=np.nan, transitions_z=np.nan, det_z=np.nan,
    lmax_z=np.nan, hurst_z=np.nan
):
    """Taxonomía longitudinal ampliada.

    Este clasificador NO modifica nunca el estado instantáneo.
    """
    if int(ref_n or 0) < 2:
        return {
            "tendencia": "Referencia longitudinal insuficiente",
            "nota": (
                "Se requieren al menos 2 sesiones previas comparables para emitir "
                "una clasificación longitudinal por Z individual."
            ),
            "reorganizacion": False,
        }

    def ge(x, t=0.80): return np.isfinite(x) and x >= t
    def le(x, t=-0.80): return np.isfinite(x) and x <= t
    def stable(x, t=0.80): return (not np.isfinite(x)) or abs(x) < t

    power_drop = le(total_z) or le(lf_z)
    complexity_rise = ge(mse_z) or ge(mde_z) or ge(c_z)
    complexity_drop = le(c_z) or le(mse_z) or le(mde_z)
    reserve_drop = le(r_z)
    reserve_gain = ge(r_z)
    vagal_gain = ge(v_z)
    alert_gain = ge(s_z)
    vagal_drop = le(v_z)
    alert_drop = le(s_z)
    entropy_rise = ge(entropy_z) or ge(transitions_z)
    entropy_drop = le(entropy_z) or le(transitions_z)
    recurrence_rise = ge(det_z) or ge(lmax_z) or ge(hurst_z)

    # 1. Desacoplamiento oscilatorio: regla prioritaria solicitada.
    if power_drop and complexity_rise:
        return {
            "tendencia": "Reorganización / Desacoplamiento Oscilatorio",
            "nota": (
                "Reconfiguración de fase respecto a la línea base previa: disminuye la "
                "potencia oscilatoria mientras aumenta la complejidad. No implica "
                "insuficiencia funcional instantánea si la reserva actual es alta."
            ),
            "reorganizacion": True,
        }

    # 2. Complejidad creciente con pérdida de soporte energético.
    if complexity_rise and reserve_drop and (entropy_rise or power_drop):
        return {
            "tendencia": "Fragmentación / Desorganización Dinámica",
            "nota": (
                "La complejidad aumenta mientras cae la reserva y/o la energía oscilatoria. "
                "Se interpreta como reorganización potencialmente costosa, no como complejidad adaptativa."
            ),
            "reorganizacion": True,
        }

    # 3. Complejidad creciente con reserva conservada o creciente.
    if complexity_rise and (reserve_gain or (np.isfinite(r_z) and r_z > -0.50)):
        return {
            "tendencia": "Ganancia de Flexibilidad / Complejidad Adaptativa",
            "nota": (
                "Aumento longitudinal de complejidad con reserva conservada o creciente. "
                "Compatible con mayor flexibilidad dinámica respecto al patrón previo."
            ),
            "reorganizacion": True,
        }

    # 4. Pérdida o ganancia global de reserva.
    if reserve_drop and (power_drop or le(v_z, -0.50)):
        return {
            "tendencia": "Pérdida Longitudinal de Reserva",
            "nota": (
                "Descenso de reserva/amplitud respecto al histórico previo, acompañado "
                "por menor potencia oscilatoria o modulación vagal."
            ),
            "reorganizacion": True,
        }

    if reserve_gain and (vagal_gain or (np.isfinite(total_z) and total_z >= 0.30)):
        return {
            "tendencia": "Ganancia de Reserva / Recuperación",
            "nota": (
                "Aumento de reserva funcional con mayor modulación vagal y/o potencia oscilatoria."
            ),
            "reorganizacion": True,
        }

    # 5. Cambios en la relación V/S.
    if vagal_gain and alert_gain:
        return {
            "tendencia": "Coactivación Longitudinal",
            "nota": (
                "V y S aumentan simultáneamente respecto al histórico previo. "
                "Describe una mayor activación conjunta, no necesariamente un deterioro."
            ),
            "reorganizacion": True,
        }

    if vagal_drop and alert_drop:
        return {
            "tendencia": "Coinhibición Longitudinal",
            "nota": (
                "V y S disminuyen simultáneamente respecto al histórico previo. "
                "Si se acompaña de pérdida de reserva, su relevancia funcional aumenta."
            ),
            "reorganizacion": True,
        }

    if np.isfinite(v_z) and np.isfinite(s_z) and (v_z - s_z) >= 0.80 and not reserve_drop:
        return {
            "tendencia": "Desplazamiento Longitudinal hacia Predominio Vagal",
            "nota": (
                "La relación V/S se desplaza hacia mayor predominio vagal respecto al patrón previo, "
                "con reserva no claramente disminuida."
            ),
            "reorganizacion": True,
        }

    if np.isfinite(v_z) and np.isfinite(s_z) and (s_z - v_z) >= 0.80 and not reserve_drop:
        return {
            "tendencia": "Desplazamiento Longitudinal hacia Alerta / Barorreflejo",
            "nota": (
                "La relación V/S se desplaza hacia mayor componente de alerta/barorreflejo "
                "respecto al patrón previo, con reserva no claramente disminuida."
            ),
            "reorganizacion": True,
        }

    # 6. Rigidificación.
    if complexity_drop and (entropy_drop or recurrence_rise):
        return {
            "tendencia": "Rigidificación Dinámica Longitudinal",
            "nota": (
                "Descenso de complejidad acompañado de menor entropía/movilidad o mayor recurrencia/persistencia."
            ),
            "reorganizacion": True,
        }

    # 7. Regulación lenta aislada.
    if ge(l_z) and stable(v_z) and stable(s_z):
        return {
            "tendencia": "Aumento de Regulación Lenta",
            "nota": (
                "Predomina un desplazamiento de procesos lentos (L/VLF/DFAα2) con V y S relativamente estables."
            ),
            "reorganizacion": True,
        }

    if le(l_z) and stable(v_z) and stable(s_z):
        return {
            "tendencia": "Disminución de Regulación Lenta",
            "nota": (
                "Descenso selectivo del componente lento respecto al histórico, sin un cambio V/S dominante."
            ),
            "reorganizacion": True,
        }

    # 8. Potencia oscilatoria aislada.
    if power_drop:
        return {
            "tendencia": "Descenso Longitudinal de Potencia Oscilatoria",
            "nota": (
                "La potencia oscilatoria ha descendido respecto al histórico previo, "
                "sin la combinación necesaria para clasificar desacoplamiento o pérdida global de reserva."
            ),
            "reorganizacion": True,
        }

    # 9. Estabilidad o transición mixta.
    if all(
        stable(x) for x in [
            total_z, lf_z, mse_z, mde_z, c_z, r_z, v_z, s_z, l_z,
            entropy_z, transitions_z, det_z, lmax_z, hurst_z
        ]
    ):
        return {
            "tendencia": "Estabilidad Longitudinal",
            "nota": "No se detectan desplazamientos individuales relevantes respecto al histórico previo.",
            "reorganizacion": False,
        }

    return {
        "tendencia": "Transición Longitudinal Mixta",
        "nota": (
            "Existen desplazamientos relevantes, pero no convergen todavía en un patrón longitudinal específico."
        ),
        "reorganizacion": True,
    }


def _autonomic_longitudinal_signature_v15522(df, row):
    """Firma longitudinal simplificada para evaluar persistencia de sesiones previas."""
    ref = _autonomic_reference_group(df, row)
    ref_n = int(len(ref))
    if ref_n < 2:
        return "Referencia longitudinal insuficiente"

    def zidx(col):
        if col not in ref.columns:
            return np.nan
        return _autonomic_progressive_z(ref[col], row.get(col, np.nan), metric=col, mode='linear')

    lf_z = _autonomic_consensus_z(ref, row, ['LF','LF_LS','LF_AR','LF_WAV_MEAN'], mode='log')
    total_z = _autonomic_consensus_z(ref, row, ['TOTAL','TOTAL_LS','TOTAL_AR'], mode='log')

    mse_vals, mde_vals = [], []
    for c in ['AUTO_MSE_short','AUTO_MSE_mid','AUTO_MSE_long']:
        if c in ref.columns:
            zz = _autonomic_progressive_z(ref[c], row.get(c, np.nan), metric=c, mode='linear')
            if np.isfinite(zz): mse_vals.append(zz)
    for c in ['AUTO_MDE_short','AUTO_MDE_mid','AUTO_MDE_long']:
        if c in ref.columns:
            zz = _autonomic_progressive_z(ref[c], row.get(c, np.nan), metric=c, mode='linear')
            if np.isfinite(zz): mde_vals.append(zz)

    result = _autonomic_classify_longitudinal_core_v15522(
        ref_n=ref_n,
        total_z=total_z,
        lf_z=lf_z,
        mse_z=float(np.mean(mse_vals)) if mse_vals else np.nan,
        mde_z=float(np.mean(mde_vals)) if mde_vals else np.nan,
        c_z=zidx('IDX_Complejidad'),
        r_z=zidx('IDX_Amplitud'),
        v_z=zidx('IDX_Vagal'),
        s_z=zidx('IDX_Rigidez'),
        l_z=zidx('IDX_Regulacion_Lenta'),
        entropy_z=zidx('WAV_ENTROPY_GLOBAL'),
        transitions_z=zidx('WAV_TRANSITIONS_PER_MIN'),
        det_z=zidx('DET'),
        lmax_z=zidx('AUTO_Lmax_rel'),
        hurst_z=zidx('Hurst'),
    )
    return result['tendencia']


def _autonomic_longitudinal_persistence_v15522(df, row, current_trend):
    """Persistencia estimada a partir de sesiones previas consecutivas con la misma firma."""
    if current_trend in ("Referencia longitudinal insuficiente", "Estabilidad Longitudinal"):
        return "No aplicable" if current_trend.startswith("Referencia") else "Estable"

    pid = str(row.get('Paciente_ID',''))
    phase = str(row.get('Fase',''))
    rec = str(row.get('Registro',''))
    row_dt = pd.to_datetime(row.get('Fecha_hora'), errors='coerce')

    prior = df[(df['Paciente_ID'].astype(str) == pid) & (df['Fase'].astype(str) == phase)].copy()
    prior = prior[prior['Registro'].astype(str) != rec]
    prior['Fecha_hora'] = pd.to_datetime(prior.get('Fecha_hora'), errors='coerce')
    if pd.notna(row_dt):
        prior = prior[(prior['Fecha_hora'].isna()) | (prior['Fecha_hora'] < row_dt)]
    prior = prior.sort_values(['Fecha_hora','Registro'], na_position='last')

    same_consecutive = 0
    # Sólo se miran las tres sesiones previas más recientes.
    for _, prow in prior.tail(3).iloc[::-1].iterrows():
        try:
            ptrend = _autonomic_longitudinal_signature_v15522(df, prow)
        except Exception:
            break
        if ptrend == current_trend:
            same_consecutive += 1
        else:
            break

    # La sesión actual cuenta como una ocurrencia del patrón.
    occurrences = 1 + same_consecutive
    if occurrences >= 3:
        return "Reorganización consolidada (≥3 sesiones consecutivas)"
    if occurrences == 2:
        return "Reorganización emergente (2 sesiones consecutivas)"
    return "Cambio agudo / transitorio (1 sesión)"


def _autonomic_classify_longitudinal_v15522(
    df, row, ref_n, total_z, lf_z, mse_z, mde_z, c_z, r_z, v_z, s_z, l_z,
    entropy_z=np.nan, transitions_z=np.nan, det_z=np.nan,
    lmax_z=np.nan, hurst_z=np.nan
):
    core = _autonomic_classify_longitudinal_core_v15522(
        ref_n=ref_n, total_z=total_z, lf_z=lf_z, mse_z=mse_z, mde_z=mde_z,
        c_z=c_z, r_z=r_z, v_z=v_z, s_z=s_z, l_z=l_z,
        entropy_z=entropy_z, transitions_z=transitions_z,
        det_z=det_z, lmax_z=lmax_z, hurst_z=hurst_z
    )
    severity = _autonomic_longitudinal_severity_v15522(
        [
            total_z, lf_z, mse_z, mde_z, c_z, r_z, v_z, s_z, l_z,
            entropy_z, transitions_z, det_z, lmax_z, hurst_z
        ],
    ...
    persistence = _autonomic_longitudinal_persistence_v15522(df, row, core['tendencia'])
    core['severidad'] = severity
    core['persistencia'] = persistence
    return core


def _autonomic_oscillatory_current_v15526(row):
    """Composición espectral ACTUAL: no usa histórico ni Z."""
    get = lambda k: _autonomic_num(row.get(k, np.nan))
    hfdom, lfdom, vlfdom, lfhf = get('HF_DOM_PCT'), get('LF_DOM_PCT'), get('VLF_DOM_PCT'), get('LF_HF')
    finite = {k:v for k,v in {'HF':hfdom,'LF':lfdom,'VLF':vlfdom}.items() if np.isfinite(v)}
    if not finite:
        return {'patron':'Perfil espectral actual no determinado',
                'nota':'No hay dominancias temporales suficientes.'}
    band = max(finite, key=finite.get); val = finite[band]
    if band == 'HF' and val >= 45:
        patron = 'Dominancia relativa HF / vagal'
        nota = 'HF domina temporalmente; esto describe reparto relativo, no potencia absoluta.'
    elif band == 'LF' and val >= 40:
        patron = 'Dominancia relativa LF / barorrefleja'
        nota = 'LF domina relativamente; no se interpreta como actividad simpática pura.'
    elif band == 'VLF' and val >= 45:
        patron = 'Dominancia relativa VLF / regulación lenta'
        nota = 'Predomina temporalmente la modulación de muy baja frecuencia.'
    else:
        patron = 'Distribución espectral relativamente mixta'
        nota = 'No hay una dominancia temporal suficientemente marcada.'
    if np.isfinite(lfhf):
        nota += f" LF/HF actual={lfhf:.2f}."
    return {'patron':patron,'nota':nota}


def _autonomic_oscillatory_longitudinal_v15526(
    ref_n, hf_z, lf_z, vlf_z, total_z,
    hf_dom_z=np.nan, lf_dom_z=np.nan, vlf_dom_z=np.nan
):
    """Cambio oscilatorio 100% LONGITUDINAL; no usa Nivel_actual ni 70/30."""
    if int(ref_n or 0) < 2:
        return {'patron':'Referencia longitudinal insuficiente',
                'nota':'Se requieren al menos 2 sesiones previas comparables.',
                'expansion':False,'contraccion':False}
    ge=lambda x,t=0.50: np.isfinite(x) and x>=t
    le=lambda x,t=-0.50: np.isfinite(x) and x<=t

    ups=sum(ge(x) for x in [hf_z,lf_z,vlf_z])
    downs=sum(le(x) for x in [hf_z,lf_z,vlf_z])
    expansion=bool(ups>=2 and ge(total_z))
    contraction=bool(downs>=2 and le(total_z))

    vagal_red=bool(ge(hf_dom_z) and (le(lf_dom_z,-0.35) or le(vlf_dom_z,-0.35)))
    lf_red=bool(ge(lf_dom_z) and (le(hf_dom_z,-0.35) or le(vlf_dom_z,-0.35)))
    vlf_red=bool(ge(vlf_dom_z) and (le(hf_dom_z,-0.35) or le(lf_dom_z,-0.35)))

    if expansion and vagal_red:
        patron='Expansión multibanda + redistribución relativa hacia HF/vagal'
        nota='Aumenta potencia absoluta en varias bandas y TOTAL, mientras la dominancia relativa se desplaza hacia HF.'
    elif expansion and lf_red:
        patron='Expansión multibanda + redistribución relativa hacia LF/barorreflejo'
        nota='Aumenta potencia global multibanda con desplazamiento relativo hacia LF.'
    elif expansion and vlf_red:
        patron='Expansión multibanda + redistribución relativa hacia VLF/lento'
        nota='Aumenta potencia global multibanda con desplazamiento relativo hacia VLF.'
    elif expansion:
        patron='Expansión multibanda'
        nota='Aumentan convergentemente al menos dos bandas y la potencia TOTAL.'
    elif contraction:
        patron='Contracción multibanda'
        nota='Disminuyen convergentemente al menos dos bandas y la potencia TOTAL.'
    elif vagal_red:
        patron='Redistribución relativa hacia HF/vagal'
        nota='La composición cambia hacia HF sin expansión global multibanda suficiente.'
    elif lf_red:
        patron='Redistribución relativa hacia LF/barorreflejo'
        nota='La composición cambia hacia LF sin expansión global multibanda suficiente.'
    elif vlf_red:
        patron='Redistribución relativa hacia VLF/lento'
        nota='La composición cambia hacia VLF sin expansión global multibanda suficiente.'
    elif all((not np.isfinite(x)) or abs(x)<0.80 for x in [hf_z,lf_z,vlf_z,total_z]):
        patron='Estabilidad oscilatoria longitudinal'
        nota='No hay desplazamientos relevantes de potencia respecto al histórico.'
    else:
        patron='Cambio oscilatorio longitudinal mixto'
        nota='Hay cambios, pero no convergen en expansión, contracción o redistribución definida.'
    return {'patron':patron,'nota':nota,'expansion':expansion,'contraccion':contraction}



def _autonomic_plain_language_v15529(row):
    """Traduce resultados técnicos a lenguaje accesible sin cambiar algoritmos."""
    trend = str(row.get('Tendencia_longitudinal', '') or '')
    severity = str(row.get('Severidad_longitudinal', '') or '')
    persistence = str(row.get('Persistencia_longitudinal', '') or '')
    reserve_now = str(row.get('Reserva_instantanea', '') or '')
    reserve_long = str(row.get('Reserva_longitudinal_relativa', '') or '')

    if trend == "Ganancia de Flexibilidad / Complejidad Adaptativa":
        evo = "Mejora importante de la capacidad de adaptación"
        evo_note = (
            "El sistema muestra más variedad y flexibilidad que en registros anteriores. "
            "La reserva se mantiene o mejora."
        )
    elif trend == "Ganancia de Reserva / Recuperación":
        evo = "Aumento de reserva y capacidad reguladora"
        evo_note = (
            "La amplitud de la variabilidad y la capacidad reguladora han aumentado respecto a registros anteriores. "
            "Este cambio longitudinal no significa por sí solo que el control actual tenga predominio vagal."
        )
    elif trend == "Pérdida Longitudinal de Reserva":
        evo = "Descenso de la reserva respecto a registros anteriores"
        evo_note = "La amplitud global ha disminuido respecto al patrón previo del paciente."
    elif trend == "Reorganización / Desacoplamiento Oscilatorio":
        evo = "Cambio importante en la forma de organizar la variabilidad"
        evo_note = (
            "El patrón actual funciona de forma diferente al histórico previo. "
            "Esto no implica por sí solo una mala función si la amplitud actual es adecuada."
        )
    elif trend == "Rigidificación Dinámica Longitudinal":
        evo = "Menor flexibilidad respecto a registros anteriores"
        evo_note = "El patrón se ha vuelto más repetitivo y menos variable que antes."
    elif trend == "Fragmentación / Desorganización Dinámica":
        evo = "Cambio menos organizado respecto a registros anteriores"
        evo_note = "Aumenta la irregularidad mientras disminuye parte de la reserva o potencia oscilatoria."
    elif trend == "Estabilidad Longitudinal":
        evo = "Sin cambios relevantes"
        evo_note = "El funcionamiento es parecido al observado en registros anteriores."
    elif trend == "Referencia longitudinal insuficiente":
        evo = "Aún no hay suficientes registros para valorar la evolución"
        evo_note = "Se necesitan más sesiones previas comparables."
    else:
        evo = trend if trend else "Evolución no determinada"
        evo_note = str(row.get('Nota_longitudinal', '') or '')

    sev_map = {
        "Muy marcada": "Cambio muy importante",
        "Marcada": "Cambio importante",
        "Moderada": "Cambio moderado",
        "Leve": "Cambio leve",
        "No significativa": "Sin cambio relevante",
    }
    magnitude = sev_map.get(severity, severity or "No determinada")

    if "≥3 sesiones" in persistence:
        persistence_plain = "Se mantiene en al menos 3 registros consecutivos"
    elif "2 sesiones" in persistence:
        persistence_plain = "Se repite en 2 registros consecutivos"
    elif "1 sesión" in persistence:
        persistence_plain = "Se observa por primera vez; pendiente de confirmar en próximos registros"
    elif persistence == "Estable":
        persistence_plain = "Patrón estable"
    elif persistence == "No aplicable":
        persistence_plain = "Aún no puede valorarse"
    else:
        persistence_plain = persistence or "No determinada"

    return {
        "modo_actual": str(row.get('Topologia_instantanea', row.get('Estado_instantaneo', 'N/D'))),
        "predominio_actual": str(row.get('Predominio_instantaneo', 'N/D')),
        "reserva_actual": reserve_now,
        "evolucion_simple": evo,
        "evolucion_nota_simple": evo_note,
        "magnitud_simple": magnitude,
        "persistencia_simple": persistence_plain,
        "reserva_long_simple": reserve_long,
    }


def _autonomic_wrap_card_v15526(title, value, body=None):
    """Tarjeta que envuelve texto largo y evita el truncamiento de st.metric."""
...
def _autonomic_dual_dashboard_v15543(
    V_abs, S_abs,
    history_V=None, history_S=None, history_labels=None,
    gray=0.20,
    topology="",
    predominance="",
    incoherent=False,
    ...
    """v15.5.43 · Diana V/S + manómetros, 100% visual y sin recalcular fisiología.

    Eje X = S (Alerta/Barorreflejo), no "simpático puro".
    Eje Y = V (Vagal).

    La zona gris usa el umbral fuzzy REAL del registro actual.
    El desacoplamiento fisiológico unilateral se representa como bandas
    cercanas a uno de los ejes, fuera de la zona central.
    La incoherencia metodológica no se fuerza a una zona geométrica:
    se muestra como aviso/etiqueta independiente.
    """
    try:
        V=float(V_abs); S=float(S_abs)
    except Exception:
        V=S=np.nan
    g=float(np.clip(gray if np.isfinite(gray) else 0.20, 0.10, 0.30))
    lim=1.0
    Vplot=float(np.clip(V,-lim,lim)) if np.isfinite(V) else 0.0
    Splot=float(np.clip(S,-lim,lim)) if np.isfinite(S) else 0.0

    fig = make_subplots(
        rows=1, cols=3,
        column_widths=[0.54,0.23,0.23],
    ...
    # --- Zonas topológicas principales, usando umbral adaptativo ---
    zones = [
        (-lim, g, -g, lim, "rgba(46,204,113,0.16)", "Reciprocidad\nvagal"),
        (g, g, lim, lim, "rgba(155,89,182,0.16)", "Coactivación"),
        (g, -lim, lim, -g, "rgba(230,126,34,0.16)", "Reciprocidad\nalerta/barorreflejo"),
        (-lim, -lim, -g, -g, "rgba(149,165,166,0.18)", "Coinhibición"),
    ]
    for x0,y0,x1,y1,fc,label in zones:
        fig.add_shape(
        ...
        fig.add_annotation(
            ...
            opacity=0.62, row=1,col=1
        )

    # Zona central fuzzy/adaptativa
    fig.add_shape(
        type="rect", x0=-g,y0=-g,x1=g,y1=g,
        fillcolor="rgba(236,240,241,0.62)",
        line=dict(color="gray",dash="dot",width=1),
        layer="below", row=1,col=1
    )
    fig.add_annotation(
        x=0,y=0,text="Zona preservada<br>/ intermedia",
        showarrow=False,font=dict(size=10),opacity=0.70,row=1,col=1
    )

    # Bandas de desacoplamiento fisiológico unilateral:
    # una rama desplazada mientras la otra queda cerca de neutro.
    neutral=0.10
    for y0,y1 in [(0.25,lim),(-lim,-0.25)]:
        fig.add_shape(
            type="rect", x0=-neutral,x1=neutral,y0=y0,y1=y1,
            fillcolor="rgba(52,152,219,0.055)",
            line=dict(color="rgba(52,152,219,0.55)",dash="dash",width=1),
            layer="below",row=1,col=1
        )
    for x0,x1 in [(0.25,lim),(-lim,-0.25)]:
        fig.add_shape(
            type="rect", y0=-neutral,y1=neutral,x0=x0,x1=x1,
            fillcolor="rgba(52,152,219,0.055)",
            line=dict(color="rgba(52,152,219,0.55)",dash="dash",width=1),
            layer="below",row=1,col=1
        )
    ...
    # Historial
    hV=[]; hS=[]; hL=[]
    if history_V is not None and history_S is not None:
        for idx,(vv,ss) in enumerate(zip(list(history_V),list(history_S))):
            try:
                vv=float(vv); ss=float(ss)
                if np.isfinite(vv) and np.isfinite(ss):
                    hV.append(float(np.clip(vv,-lim,lim)))
                    hS.append(float(np.clip(ss,-lim,lim)))
                    if history_labels is not None and idx < len(history_labels):
                        hL.append(str(history_labels[idx]))
                    else:
                        hL.append(f"Registro {idx+1}")
            except Exception:
                pass
    ...
    if hV and hS:
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines+markers",
                name="Trayectoria",
                line=dict(color="rgba(44,62,80,0.35)",width=2),
                marker=dict(size=6,color="rgba(44,62,80,0.48)"),
                text=hL,
                hovertemplate="%{text}<br>S=%{x:.2f}<br>V=%{y:.2f}<extra></extra>"
            ),
            row=1,col=1
        )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers",
            name="Estado actual",
            marker=dict(size=17,color="#2C3E50",symbol="cross",
                        line=dict(width=2,color="white")),
            hovertemplate=(
                f"<b>Estado actual</b><br>"
                f"V={V:+.3f}<br>S={S:+.3f}<br>"
                f"{topology}<br>{predominance}<extra></extra>"
            )
        ),
        row=1,col=1
    )
    ...
    # Animación temporal: sólo mueve un marcador; mantiene diana y gauges.
    if animate and len(hV) >= 2:
        fig.add_trace(
            go.Scatter(
                x=[hS[0]],y=[hV[0]],
                mode="markers",
                name="Recorrido temporal",
                marker=dict(size=13,color="#34495E",symbol="circle",
                            line=dict(width=2,color="white")),
                hoverinfo="skip"
            ),
            row=1,col=1
        )
        anim_trace_index=len(fig.data)-1
        frames=[]
        for k,(xx,yy,ll) in enumerate(zip(hS,hV,hL)):
            frames.append(
                go.Frame(
                    data=[go.Scatter(
                        ...
                        marker=dict(size=13,color="#34495E",symbol="circle",
                                    line=dict(width=2,color="white")),
                        hovertemplate=f"{ll}<br>S={xx:.2f}<br>V={yy:.2f}<extra></extra>"
                    )],
                    traces=[anim_trace_index],
                    name=str(k)
                )
            )
        fig.frames=frames
            ...
            updatemenus=[dict(
                ...
                buttons=[
                    dict(
                        label="▶ Reproducir",
                        method="animate",
                        args=[None,{"frame":{"duration":550,"redraw":False},
    ...
    fig.add_trace(
        go.Indicator(
            ...
            gauge={
                ...
                "bar":{"color":"#2ECC71","thickness":0.30},
                "steps":[
                    {"range":[-1,-g],"color":"rgba(231,76,60,0.12)"},
                    {"range":[-g,g],"color":"rgba(189,195,199,0.28)"},
                    {"range":[g,1],"color":"rgba(46,204,113,0.18)"}
                ],
                "threshold":{
                    "line":{"color":"#2C3E50","width":3},
                    "thickness":0.75,"value":Vplot
                }
            }
        ),
        row=1,col=2
    ...
    fig.add_trace(
        go.Indicator(
            ...
            gauge={
                "axis":{"range":[-1,1],"tickwidth":1},
                "bar":{"color":"#E67E22","thickness":0.30},
                "steps":[
                    {"range":[-1,-g],"color":"rgba(189,195,199,0.22)"},
                    {"range":[-g,g],"color":"rgba(189,195,199,0.28)"},
                    {"range":[g,1],"color":"rgba(230,126,34,0.18)"}
                ],
                "threshold":{
                    "line":{"color":"#2C3E50","width":3},
                    "thickness":0.75,"value":Splot
                }
            }
        ),
        row=1,col=3
    ...
    fig.update_xaxes(
        title_text="S · Alerta / Barorreflejo",
        range=[-1,1],zeroline=True,zerolinewidth=2,zerolinecolor="gray",
    ...
    subtitle = f"{topology} · {predominance}" if topology or predominance else ""
    if incoherent:
        subtitle += " · ⚠ patrón complejo/incoherente"

    fig.update_layout(
        height=520,
        showlegend=False,
        template="plotly_white",
        margin=dict(l=35,r=25,t=80,b=75),
        title=dict(text=subtitle,x=0.02,xanchor="left",font=dict(size=14)),
        uirevision="autonomic_dual_v15543"
    )
    return fig


def autonomic_control_assessment(df, row, record_data=None):
    """v15.5.26 · Control autonómico en planos estrictamente independientes.

    1) Estado actual: 100% Nivel_actual V/S/L/C/R.
    2) Estado integrado: 70% actual + 30% longitudinal.
    3) Longitudinal: 100% Z/Δ respecto a sesiones previas.
    4) Patrón oscilatorio: separa reparto actual de expansión/contracción longitudinal.

    El plano integrado es una síntesis secundaria y nunca redefine los otros dos.
    """
    ref = _autonomic_reference_group(df, row)
    get = lambda k: _autonomic_num(row.get(k, np.nan))
    z = lambda k, mode='linear': _autonomic_progressive_z(
        ref[k] if k in ref.columns else [], get(k), metric=k, mode=mode
    )

    zmap = {}
    linear_cols = [
        'RMSSD','SD1','HF_DOM_PCT','pNN50','DFA_alpha1','LF_DOM_PCT','MeanHR','LF_HF','IDX_Rigidez',
    ...
    C_long = _autonomic_weighted_score([
        ...
        (mse_cons,0.45),(mde_cons,0.45),
        (-zmap['DET'] if np.isfinite(zmap['DET']) else np.nan,0.25),
        (-zmap['AUTO_Lmax_rel'] if np.isfinite(zmap['AUTO_Lmax_rel']) else np.nan,0.25),
        (zmap['IDX_Complejidad'],0.35)
    ])
    R_long = _autonomic_weighted_score([
        (zmap['SDNN'],1.00),(zmap['SD2'],0.90),(total_cons,0.80),(zmap['IDX_Amplitud'],0.45)
    ])

    # Plano actual: EXCLUSIVAMENTE valores absolutos/índices integrados de la sesión.
    _patient_id_v15528 = str(row.get('Paciente_ID', '') or row.get('Registro',''))
    _record_id_v15533 = str(row.get('Registro',''))
    _profile_v15528 = _resolve_reference_profile_v15533(_patient_id_v15528, _record_id_v15533)
    abs_levels = _autonomic_abs_vector_levels_v15542(
        row,
        age_years=_profile_v15528.get('age_years'),
        sex=_profile_v15528.get('sex', 'No especificado')
    )
    V_abs, S_abs, L_abs, C_abs, R_abs = (abs_levels[k] for k in ['V','S','L','C','R'])
    _ctx_v15528 = abs_levels.get('context', {})

    # Nivel efectivo se conserva sólo como información secundaria 70/30.
    V_eff = _autonomic_fuse_planes(V_abs, V_long)
    S_eff = _autonomic_fuse_planes(S_abs, S_long)
    L_eff = _autonomic_fuse_planes(L_abs, L_long)
    C_eff = _autonomic_fuse_planes(C_abs, C_long)
    R_eff = _autonomic_fuse_planes(R_abs, R_long)

    Q = _autonomic_quality_score(row)
    ref_n = int(len(ref))

    oscillatory_now = _autonomic_oscillatory_current_v15526(row)

    instant = _autonomic_classify_instantaneous_v15542(
        V_abs, S_abs, L_abs, C_abs, R_abs,
        hfdom=get('HF_DOM_PCT'),
        lfdom=get('LF_DOM_PCT'),
        lfhf=get('LF_HF'),
        idx_amp=get('IDX_Amplitud'),
        oscillatory_current=oscillatory_now,
        context_scores=_ctx_v15528.get('scores', {}),
        quality=Q
    )
    longitudinal = _autonomic_classify_longitudinal_v15522(
        df=df,
        row=row,
        ref_n=ref_n,
        total_z=total_cons,
        ...
        s_z=S_long,
        l_z=L_long,
        entropy_z=zmap['WAV_ENTROPY_GLOBAL'],
        transitions_z=zmap['WAV_TRANSITIONS_PER_MIN'],
        det_z=zmap['DET'],
        lmax_z=zmap['AUTO_Lmax_rel'],
        hurst_z=zmap['Hurst'],
    )

    oscillatory_long = _autonomic_oscillatory_longitudinal_v15526(
        ref_n=ref_n,
        hf_z=hf_cons, lf_z=lf_cons, vlf_z=vlf_cons, total_z=total_cons,
        hf_dom_z=zmap['HF_DOM_PCT'],
        lf_dom_z=zmap['LF_DOM_PCT'],
        vlf_dom_z=zmap['VLF_DOM_PCT'],
    )

    integrated = _autonomic_classify_integrated_v15527(V_eff, S_eff, L_eff, C_eff, R_eff)

    # Confianza del estado instantáneo: disponibilidad del plano actual + calidad.
    abs_available = np.mean([np.isfinite(x) for x in [V_abs,S_abs,L_abs,C_abs,R_abs]])
    conf = float(np.clip(100.0 * (0.55 + 0.45*abs_available) * (0.70 + 0.30*Q), 0, 100))

    # Organización actual descriptiva: no usa longitudinal.
    if np.isfinite(C_abs):
        dyn_now = "flexible/alta movilidad" if C_abs >= 0.25 else (
            "rígida/baja movilidad" if C_abs <= -0.35 else "intermedia"
        )
    else:
        dyn_now = "no determinada"

    # Compatibilidad hacia atrás: las claves antiguas siguen existiendo,
    # pero Estado_control_autonomico y Forma_base_A_F representan ahora el plano instantáneo.
    reason_now = (
        "Plano instantáneo V/S/L/C/R="
        + "/".join(f"{x:+.2f}" if np.isfinite(x) else "N/D" for x in [V_abs,S_abs,L_abs,C_abs,R_abs])
        + f" · vectorial={instant['predominio_vectorial']}"
        + f" · espectral={instant['apoyo_espectral']}"
        + f" · resultado={instant['topologia']} / {instant['predominio']} · {instant['reserva']}"
    )
    reason_long = (
        "Plano longitudinal V/S/L/C/R="
        + "/".join(f"{x:+.2f}" if np.isfinite(x) else "N/D" for x in [V_long,S_long,L_long,C_long,R_long])
        + f" · TOTAL_Z={total_cons:+.2f}" if np.isfinite(total_cons) else ""
    )

    return {
        # Nuevos componentes separados
        'Estado_instantaneo': instant['estado'],
        'Topologia_instantanea': instant['topologia_mostrada'],
        'Familia_topologica_instantanea': instant['topologia'],
        'Calificador_topologico_instantaneo': instant['calificador_topologico'],
        'Predominio_instantaneo': instant['predominio'],
        'Reserva_instantanea': _reserve_context_label_v15528(R_abs, _ctx_v15528),
        'Reserva_longitudinal_relativa': _autonomic_reserve_relative_label_v15525(R_long),
        'Edad_referencia': _profile_v15528.get('age_years'),
        'Sexo_referencia': _profile_v15528.get('sex', 'No especificado'),
        'Grupo_edad_referencia': _ctx_v15528.get('group'),
        'Fuente_perfil_referencia': _profile_v15528.get('source', 'paciente'),
        'Detalle_normalizacion_edad': json.dumps(_ctx_v15528.get('details', []), ensure_ascii=False),
        'Predominio_vectorial_pre_espectral': instant['predominio_vectorial'],
        'Apoyo_espectral_transversal': instant['apoyo_espectral'],
        'Discordancia_espectral_transversal': instant['discordancia_espectral'],
        'HF_DOM_actual_control': instant['HF_DOM_actual'],
        'LF_DOM_actual_control': instant['LF_DOM_actual'],
        'LF_HF_actual_control': instant['LF_HF_actual'],
        'Fuente_predominio_actual': instant.get('fuente_predominio', 'N/D'),
        'Explicacion_predominio_actual': instant.get('explicacion_predominio', ''),
        'Discordancia_espectral_actual': bool(instant.get('discordancia_espectral', False)),
        'Nota_convergencia_actual': instant['Nota_convergencia_actual'],
        'Score_Coactivacion_actual': instant.get('Score_Coactivacion', np.nan),
        'Score_Coactivacion_bruto_actual': instant.get('Score_Coactivacion_bruto', np.nan),
        'Score_Reciprocidad_actual': instant.get('Score_Reciprocidad', np.nan),
        'Score_Reciprocidad_bruto_actual': instant.get('Score_Reciprocidad_bruto', np.nan),
        'Score_Coinhibicion_actual': instant.get('Score_Coinhibicion', np.nan),
        'Score_Coinhibicion_bruto_actual': instant.get('Score_Coinhibicion_bruto', np.nan),
        'Score_Desacoplamiento_actual': instant.get('Score_Desacoplamiento', np.nan),
        'Score_Desacoplamiento_bruto_actual': instant.get('Score_Desacoplamiento_bruto', np.nan),
        'Score_Intermedio_actual': instant.get('Score_Intermedio', np.nan),
        'Score_Incoherencia_actual': instant.get('Score_Incoherencia', np.nan),
        'Incoherencia_detectada': instant.get('Incoherencia_detectada', False),
        'Umbral_gris_actual': instant.get('Umbral_gris_actual', np.nan),
        'Pertenencia_V_actual': json.dumps(instant.get('Pertenencia_V', {}), ensure_ascii=False),
        'Pertenencia_S_actual': json.dumps(instant.get('Pertenencia_S', {}), ensure_ascii=False),
        'Coactivacion_permitida': instant.get('Coactivacion_permitida', False),
        'Apoyos_coactivacion_preservada': instant.get('Apoyos_coactivacion_preservada', 0),
        'Reciprocidad_permitida': instant.get('Reciprocidad_permitida', False),
        'Desacoplamiento_permitido': instant.get('Desacoplamiento_permitido', False),
        'Estado_V_actual': instant.get('Estado_V_actual', 'N/D'),
        'Estado_S_actual': instant.get('Estado_S_actual', 'N/D'),
        'Contribucion_VS_topologia_pct': instant.get('Contribucion_VS_pct', 70),
        'Contribucion_LCR_topologia_pct': instant.get('Contribucion_LCR_pct', 30),
        'Equilibrio_Lento_actual': instant.get('Equilibrio_Lento_actual', np.nan),
        'Carga_Lenta_elevada_actual': instant.get('Carga_Lenta_elevada_actual', False),
        'Expresion_Lenta_reducida_actual': instant.get('Expresion_Lenta_reducida_actual', False),
        'Actividad_multibanda_actual': instant.get('Actividad_multibanda_actual', False),
        'Actividad_multibanda_actual_fuerte': instant.get('Actividad_multibanda_actual_fuerte', False),
        'Coinhibicion_permitida': instant.get('Coinhibicion_permitida', False),
        'Motivo_compuerta_coinhibicion': instant.get('Motivo_compuerta_coinhibicion', ''),
        'Patron_oscilatorio_actual': oscillatory_now['patron'],
        'Nota_patron_oscilatorio_actual': oscillatory_now['nota'],
        'Patron_oscilatorio_longitudinal': oscillatory_long['patron'],
        'Nota_patron_oscilatorio_longitudinal': oscillatory_long['nota'],
        'Expansion_multibanda_longitudinal': bool(oscillatory_long['expansion']),
        'Contraccion_multibanda_longitudinal': bool(oscillatory_long['contraccion']),
        'Estado_integrado_70_30': integrated['estado_integrado'],
        'Topologia_integrada_70_30': integrated['topologia_integrada'],
        'Predominio_integrado_70_30': integrated['predominio_integrado'],
        'Reserva_integrada_70_30': integrated['reserva_integrada'],
        'Tendencia_longitudinal': longitudinal['tendencia'],
        'Severidad_longitudinal': longitudinal['severidad'],
        'Persistencia_longitudinal': longitudinal['persistencia'],
        'Nota_longitudinal': longitudinal['nota'],
        'Reorganizacion_desacoplamiento_oscilatorio': bool(
            longitudinal['tendencia'] == 'Reorganización / Desacoplamiento Oscilatorio'
        ),
        'Reorganizacion_longitudinal': bool(longitudinal['reorganizacion']),

        # Compatibilidad con exportaciones previas
        'Estado_control_autonomico': instant['estado'],
        'Forma_base_A_F': instant['topologia'],
        'Predominio_autonomico': instant['predominio'],
        'Amplitud_reserva': instant['reserva'],
        'Organizacion_dinamica': dyn_now,
        'Regla_estricta_cumplida': instant['topologia'] != "Transición Intermedia",
        'Confianza_clasificacion_pct': conf,

        # Dos planos completos
        'Vector_V_Z': V_long, 'Vector_S_Z': S_long, 'Vector_L_Z': L_long, 'Vector_C_Z': C_long, 'Vector_R_Z': R_long,
        'Vector_V_Actual': V_abs, 'Vector_S_Actual': S_abs, 'Vector_L_Actual': L_abs, 'Vector_C_Actual': C_abs, 'Vector_R_Actual': R_abs,
        'Rigidez_actual_independiente': get('IDX_Rigidez'),
        'Vector_V_Efectivo': V_eff, 'Vector_S_Efectivo': S_eff, 'Vector_L_Efectivo': L_eff, 'Vector_C_Efectivo': C_eff, 'Vector_R_Efectivo': R_eff,
        ...
        'Atractor': _autonomic_attractor_label(row.get('Registro',''), record_data),
        'Coste_alostatico': "Evaluar en el plano longitudinal; no determina la etiqueta instantánea",
        'Interpretacion_fisiologica': (
            f"Estado transversal actual: {instant['topologia_mostrada']} / {instant['predominio']}. "
            f"{_reserve_context_label_v15528(R_abs, _ctx_v15528)}. "
            "La normalización transversal usa referencias por edad/sexo y no se modifica por Z-scores longitudinales."
        ),
        'Implicacion_clinica': (
            "Interpretar la reserva y el modo de control actual junto con síntomas, contexto y tolerancia funcional. "
            "La tendencia longitudinal se informa por separado y no invalida una reserva instantánea alta."
        ),
        'Justificacion': reason_now + " · " + longitudinal['tendencia'] + " · " + longitudinal['nota'],
        'Justificacion_instantanea': reason_now,
        'Justificacion_longitudinal': longitudinal['tendencia'] + " · " + longitudinal['nota'],

        'Referencia_n': ref_n,
        'Referencia_metodo': _autonomic_reference_method(ref_n),
    ...
    for rec in list(records or []):
        ...
        if m_rec >= 3:
            ...
            fig3.update_layout(title=f"Atractor reconstruido 3D · {rec}", height=650,
                               scene=dict(xaxis_title="RR(t)", yaxis_title="RR(t-τ)", zaxis_title="RR(t-2τ)", uirevision="attractor_camera_live_v15514"),
                               showlegend=False)
...
def _bundle_record_keys_v15518(patient_id):
    """Devuelve las claves actualmente pertenecientes al bundle longitudinal del paciente."""
    try:
        _bundles = list_patient_analysis_bundles()
        if _bundles is None or _bundles.empty:
            return set()
        _patient = infer_patient_id(patient_id)
        _match = _bundles[
            _bundles["patient_id"].astype(str).map(infer_patient_id) == _patient
        ]
        if _match.empty:
            return set()
        _row = _match.sort_values("saved_at").iloc[-1]
        try:
            return {str(x) for x in json.loads(_row.get("record_keys_json") or "[]")}
        except Exception:
            return set()
    except Exception:
        return set()
...
def _force_cloud_persist_and_verify_v15518(reason="explicit_analysis_save"):
    """Fuerza PostgreSQL y verifica que la imagen remota sea idéntica a la SQLite local."""
    if not _persistent_db_url_v1540():
        return False, "PostgreSQL persistente no está configurado"
    if not LONGITUDINAL_DB_PATH.exists():
        return False, "SQLite longitudinal local no existe"

    _ok = _cloud_checkpoint_v1540(reason, force=True)
    if not _ok:
        _err = (_cloud_state_v1540().get("last_error") or "sin detalle")
        return False, f"fallo al sincronizar PostgreSQL: {_err}"

    # Garantizar que WAL quede incorporado antes de calcular SHA local.
    try:
        with sqlite3.connect(LONGITUDINAL_DB_PATH) as _con:
    ...
    try:
        _local_raw = LONGITUDINAL_DB_PATH.read_bytes()
        _local_sha = hashlib.sha256(_local_raw).hexdigest()
    except Exception as _exc:
        return False, f"no se pudo verificar SQLite local: {_exc}"

    _remote = _cloud_get_object_v1540(CLOUD_STORE_KEY_V1540)
    if not _remote:
        return False, "PostgreSQL no devolvió una copia persistente verificable"

    _remote_sha = str(_remote.get("sha256") or "")
    if _remote_sha != _local_sha:
        return False, "la copia de PostgreSQL no coincide con la SQLite local después del guardado"

    # Verificación adicional: la copia remota debe seguir siendo una SQLite válida.
    try:
        _payload = bytes(_remote.get("payload") or b"")
        if not _payload.startswith(b"SQLite format 3\x00"):
            return False, "la copia remota no contiene una SQLite válida"
        _tmp = LONGITUDINAL_DB_PATH.with_suffix(".v15518.verify.tmp")
        _tmp.write_bytes(_payload)
        with sqlite3.connect(_tmp) as _chk:
            _qc = _chk.execute("PRAGMA quick_check").fetchone()
        try:
            _tmp.unlink(missing_ok=True)
        except Exception:
            pass
        if not _qc or str(_qc[0]).lower() != "ok":
            return False, f"quick_check remoto no válido: {_qc}"
    except Exception as _exc:
        return False, f"no se pudo validar la copia remota: {_exc}"

    _state = _cloud_state_v1540()
    _state["dirty"] = False
    _state["dirty_reasons"] = []
    return True, "PostgreSQL sincronizado y verificado por SHA-256 + quick_check"
...
def _persist_one_complete_analysis_v15324(_rec, patient_override=None):
    ...
    # v15.5.19 · salvaguarda de guardado incremental:
    # El panel "Guardar análisis" se ejecuta antes que algunos controles de edición
    # de segmentación dentro del script. Si records_results quedó vacío por el estado
    # del rerun anterior, al pulsar Guardar recalculamos exclusivamente ESTE registro
    # con las ventanas y fases que existen AHORA en st.session_state.
    if _dfsv is None or _dfsv.empty:
        try:
            _windows_now_v15519 = get_record_windows(
                st.session_state.get("global_windows_v50", global_windows_safe),
                st.session_state.get("record_windows_v50", record_windows_safe),
                _rec,
                bool(st.session_state.get("use_independent_v70", use_independent)),
            )
            _active_now_v15519 = list(st.session_state.get("active_phases_v50", list(active_phases)) or [])
            _df_now_v15519, _seg_now_v15519, _valid_now_v15519 = calculate_record_cached_v15213(
                _dsv.get("rr"),
                _windows_now_v15519,
                tuple(_active_now_v15519),
                int(min_rr),
                bool(include_rqa),
                bool(include_hvg),
                st.session_state.get("mse_zero_policy", "nan"),
            ...
            if _df_now_v15519 is not None and not _df_now_v15519.empty:
                _dfsv = _df_now_v15519.copy()
                # Actualizar las estructuras en RAM para que el resto de la sesión vea
                # exactamente el mismo resultado que se va a congelar.
                records_results[_rec] = _dfsv
                records_segments[_rec] = _seg_now_v15519
                records_valid[_rec] = _valid_now_v15519
        except Exception as _recalc_exc_v15519:
            return False, f"no se pudo recalcular el registro antes de guardar: {_recalc_exc_v15519}"

    if _dfsv is None or _dfsv.empty:
        # Diagnóstico explícito: evita el genérico "sin resultados válidos".
        _windows_diag_v15519 = get_record_windows(
            st.session_state.get("global_windows_v50", global_windows_safe),
            st.session_state.get("record_windows_v50", record_windows_safe),
            _rec,
            bool(st.session_state.get("use_independent_v70", use_independent)),
        )
        _active_diag_v15519 = list(st.session_state.get("active_phases_v50", list(active_phases)) or [])
        _parts_v15519 = []
        for _ph_v15519 in PHASES:
            _w_v15519 = _windows_diag_v15519.get(_ph_v15519)
            if _w_v15519 is None:
                continue
            try:
                _seg_v15519 = cut_segment(_dsv.get("rr"), float(_w_v15519[0]), float(_w_v15519[1]))
                _parts_v15519.append(
                    f"{_ph_v15519}: {len(_seg_v15519)} RRi "
                    f"({'activa' if _ph_v15519 in _active_diag_v15519 else 'NO activa'})"
                )
            except Exception:
                pass
        _diag_v15519 = "; ".join(_parts_v15519) if _parts_v15519 else "ninguna ventana definida"
        return False, (
            f"sin resultados válidos tras recalcular. Mínimo RRi={int(min_rr)}; "
            f"fases activas={', '.join(_active_diag_v15519) if _active_diag_v15519 else 'ninguna'}; "
            f"{_diag_v15519}"
        )
    ...
    try:
        ...
        _bundle_id_saved, _bundle_keys_saved = save_patient_analysis_bundle(
            _bundle_patient, [_rksv],
            config={
                'last_session_version':'15.5.18', 'explicit_save':True, 'complete_snapshot':True,
                'incremental_append': bool(patient_override), 'incremental_target_patient': _bundle_patient
            },
            include_all_snapshotted=True
        )
    except Exception as _bundle_exc:
        return False, f"snapshot escrito, pero falló la actualización del histórico: {_bundle_exc}"
    ...
    # v15.5.18: el registro debe pertenecer realmente al bundle del paciente.
    _bundle_keys_check = _bundle_record_keys_v15518(_bundle_patient)
    if str(_rksv) not in _bundle_keys_check:
        return False, "snapshot recuperable, pero el registro no quedó incorporado al histórico del paciente"

    return True, f"{len(_verify['results'])} fase(s) congelada(s) · histórico verificado"
...
if record_data and not globals().get('_frozen_hist_v1537', False):
    with st.expander("Guardar análisis actual en el historial", expanded=True):
        ...
        st.caption(
            ...
            "Usa «Guardar TODOS» antes de cerrar la app. v15.5.19 recalcula de forma segura el registro si es necesario y fuerza/verifica la copia PostgreSQL en ese mismo guardado; en modo incremental NO necesitas volver a cargar los registros anteriores."
        )
        ...
        if len(_valid_to_freeze_v15324) < len(_save_targets_v15327):
            st.caption(
                "v15.5.19: si un registro nuevo aparece todavía con 0 resultados en este resumen, "
                "«Guardar TODOS» fuerza un recálculo final con las ventanas/fases actuales antes de decidir si puede guardarse."
            )
        ...
        with _c_all:
            if st.button("Guardar TODOS los análisis calculados", key="save_all_snapshots_btn_v15324", type="primary", use_container_width=True):
                ...
                # v15.5.18: fotografía del bundle antes del guardado incremental.
                _before_keys_v15518 = (
                    _bundle_record_keys_v15518(_append_target_v15325)
                    if _append_target_v15325 else set()
                )
                _saved_keys_v15518 = set()
                ...
                for _rec in list(_save_targets_v15327):
                    ...
                    if _ok:
                        _d_v15518 = record_data.get(_rec, {})
                        _rk_v15518 = _d_v15518.get('record_key') or _raw_record_key(_rec, _d_v15518.get('filename', _rec))
                        _saved_keys_v15518.add(str(_rk_v15518))
                    _saved += int(_ok)
                ...
                _local_bundle_ok_v15518 = True
                _bundle_detail_v15518 = ""
                _after_count_v15518 = None

                if _saved == len(_save_targets_v15327) and _append_target_v15325:
                    _after_keys_v15518 = _bundle_record_keys_v15518(_append_target_v15325)
                    _expected_keys_v15518 = _before_keys_v15518 | _saved_keys_v15518
                    _missing_keys_v15518 = _expected_keys_v15518 - _after_keys_v15518
                    _after_count_v15518 = len(_after_keys_v15518)

                    if _missing_keys_v15518:
                        _local_bundle_ok_v15518 = False
                        _bundle_detail_v15518 = (
                            f"El bundle no contiene {len(_missing_keys_v15518)} registro(s) que debían quedar anexados."
                        )
                    elif len(_after_keys_v15518) < len(_expected_keys_v15518):
                        _local_bundle_ok_v15518 = False
                        _bundle_detail_v15518 = "El contador del histórico no aumentó hasta el valor esperado."

                if _saved == len(_save_targets_v15327) and _local_bundle_ok_v15518:
                    # v15.5.18: Guardar TODOS = persistir AHORA, no dejar cambios amarillos pendientes.
                    _cloud_ok_v15518, _cloud_msg_v15518 = _force_cloud_persist_and_verify_v15518(
                        "explicit_save_all_v15518"
                    )

                    if _cloud_ok_v15518:
                        if _append_target_v15325:
                            _new_unique_v15518 = len(_saved_keys_v15518 - _before_keys_v15518)
                            st.success(
                                f"Visita añadida y persistida en nube. Histórico de "
                                f"{_append_target_v15325.replace('_',' ').title()}: "
                                f"{_after_count_v15518} registro(s) totales. "
                                f"{_new_unique_v15518} registro(s) nuevos incorporados y verificados. "
                                f"{_cloud_msg_v15518}."
                            )
                        else:
                            st.success(
                                f"Histórico completo guardado y persistido en nube: "
                                f"{_saved}/{len(_save_targets_v15327)} registros verificados. "
                                f"{_cloud_msg_v15518}."
                            )
                    else:
                        st.error(
                            "Los análisis se escribieron localmente, pero NO se confirma persistencia duradera en PostgreSQL. "
                            f"No cierres la app todavía. Detalle: {_cloud_msg_v15518}"
                        )
                elif _saved == len(_save_targets_v15327) and not _local_bundle_ok_v15518:
                    st.error(
                        "Los snapshots se escribieron, pero el histórico longitudinal NO quedó consolidado correctamente. "
                        f"No se muestra éxito ni se considera persistido. {_bundle_detail_v15518}"
                    )
                elif _saved:
                    st.warning(
                        f"Guardado parcial: {_saved}/{len(_save_targets_v15327)} registros. "
                        "No se ha realizado la confirmación final en PostgreSQL."
                    )
                else:
        ...
        with _c_one:
            ...
            if st.button("Guardar registro seleccionado", key="save_snapshot_btn_v1537", use_container_width=True):
                try:
                    ...
                    _ok, _msg = False, str(_e)
                if _ok:
                    _cloud_ok_one_v15518, _cloud_msg_one_v15518 = _force_cloud_persist_and_verify_v15518(
                        "explicit_save_one_v15518"
                    )
                    if _cloud_ok_one_v15518:
                        st.success(f"{_save_snapshot_rec_v1537}: {_msg}. {_cloud_msg_one_v15518}.")
                    else:
                        st.error(
                            f"{_save_snapshot_rec_v1537}: guardado localmente, pero NO confirmado en PostgreSQL. "
                            f"No cierres la app todavía. {_cloud_msg_one_v15518}"
                        )
                else:
...
with tab4:
    ...
    if _attr_records:
        ...
        _anim_c1, _anim_c2, _anim_c3 = st.columns([1.35, 1, 1])
        with _anim_c1:
            _animate_attr = st.toggle(
                "▶ Animar trayectoria(s)", value=False, key="attractor_animate_v15514",
                help="Con uno o varios registros: mantiene visible cada trayectoria y hace avanzar un punto móvil por cada atractor siguiendo su orden temporal."
            )
        with _anim_c2:
            _anim_speed_label = st.selectbox(
                "Velocidad", ["Lenta", "Media", "Rápida"], index=1, key="attractor_anim_speed_v15514",
                disabled=not _animate_attr
            )
        with _anim_c3:
            _anim_frames = st.select_slider(
                "Suavidad (frames)", options=[60,120,180,240], value=180, key="attractor_anim_frames_v15514",
                disabled=not _animate_attr, help="Más frames = movimiento más continuo, pero mayor carga gráfica."
            )
        _anim_speed_ms = {"Lenta": 110, "Media": 55, "Rápida": 25}.get(_anim_speed_label, 55)
        if _animate_attr:
            if len(_attr_records) > 1:
                st.caption("Animación multirregistro: cada atractor conserva su trayectoria completa y tiene su propio punto móvil. Todos avanzan sincronizados entre el inicio y el final de cada trayectoria, aunque tengan distinto número de puntos. En 3D puedes rotar y hacer zoom sin pulsar Pausa: durante el gesto la animación hace una micro-pausa técnica y se reanuda automáticamente al soltar. Cada registro mantiene un color propio, incluido su punto móvil.")
            else:
                st.caption("La línea conserva todo el atractor y el punto móvil recorre cronológicamente sus estados. En 3D puedes rotar, hacer zoom y desplazar la cámara incluso durante ▶ Reproducir. Mientras mantienes el ratón/gesto sobre la cámara, el punto móvil deja de redibujarse momentáneamente para no bloquear la interacción; al soltar, se sincroniza con el punto temporal actual y continúa automáticamente.")
        ...
        _valid_attr = 0
        _attr_cache_v15323 = {}
        _anim_tracks3_v15514 = []
        _anim_tracks2_v15514 = []
        for _rec in _attr_records:
            ...
            _anim_tracks2_v15514.append((_short_record_label(_rec, 28), _show))
            if _m_rec >= 3:
                _anim_tracks3_v15514.append((_short_record_label(_rec, 28), _show))
        ...
        # v15.5.14 · colores persistentes por registro en superposición.
        # El mismo color se reutiliza para trayectoria y punto móvil.
        _attr_palette_v15514 = list(px.colors.qualitative.Plotly) + list(px.colors.qualitative.Safe)
        for _i_col, _tr_col in enumerate(_fig3.data):
            _c_col = _attr_palette_v15514[_i_col % len(_attr_palette_v15514)]
            try:
                _tr_col.update(line=dict(color=_c_col, width=3))
            except Exception:
                pass
        for _i_col, _tr_col in enumerate(_fig2.data):
            _c_col = _attr_palette_v15514[_i_col % len(_attr_palette_v15514)]
            try:
                _tr_col.update(line=dict(color=_c_col, width=2.5))
            except Exception:
                pass
        ...
        if _valid_attr:
            if _view_mode == "Comparación superpuesta":
                if any((v.get("ok") and int(v.get("m", 0)) >= 3) for v in _attr_cache_v15323.values()):
                    ...
                    if _animate_attr:
                        if len(_anim_tracks3_v15514) >= 2:
                            st.caption(f"▶ Reproducción conjunta activa: {len(_anim_tracks3_v15514)} atractores 3D sincronizados.")
                        _render_live_attractor_3d(
                            _fig3, _anim_tracks3_v15514,
                            frame_duration_ms=_anim_speed_ms,
                            frame_cap=int(_anim_frames),
                            height=680
                        )
                    else:
                        st.plotly_chart(_fig3, use_container_width=True, key="attractor_compare_3d_v15514")
                    _attractor_video_export_ui(
                        _anim_tracks3_v15514,
                        key_prefix="attractor_video_superposed_v15515"
                    )
                if _animate_attr:
                    _fig2 = _configure_multi_attractor_animation(_fig2, _anim_tracks2_v15514, dims=2, frame_duration_ms=_anim_speed_ms, frame_cap=int(_anim_frames))
                _fig2.update_layout(
                ...
                # v15.5.13: si hay >=2 atractores 3D válidos y la animación está activa,
                # ofrecer además un reproductor conjunto sincronizado. Así no depende de
                # que el usuario cambie manualmente a "Comparación superpuesta".
                if _animate_attr and len(_anim_tracks3_v15514) >= 2:
                    st.markdown("#### ▶ Reproducción sincronizada de los atractores seleccionados")
                    st.caption("Los puntos móviles avanzan simultáneamente. Cada trayectoria se normaliza a 0–100% de su recorrido para que registros de distinta longitud comiencen y terminen juntos.")
                    _fig_sync = go.Figure()
                    _sync_palette_v15514 = list(px.colors.qualitative.Plotly) + list(px.colors.qualitative.Safe)
                    for _i_sync, (_nm_sync, _tr_sync) in enumerate(_anim_tracks3_v15514):
                        _arr_sync = np.asarray(_tr_sync, dtype=float)
                        if _arr_sync.ndim == 2 and len(_arr_sync) >= 2 and _arr_sync.shape[1] >= 3:
                            _sync_color_v15514 = _sync_palette_v15514[_i_sync % len(_sync_palette_v15514)]
                            _fig_sync.add_trace(go.Scatter3d(
                                x=_arr_sync[:,0], y=_arr_sync[:,1], z=_arr_sync[:,2],
                                mode="lines", name=str(_nm_sync),
                                line=dict(color=_sync_color_v15514, width=3)
                            ))
                    _fig_sync.update_layout(
                        title="Reproducción conjunta · atractores 3D seleccionados",
                        height=680,
                        scene=dict(
                            xaxis_title="RR(t)", yaxis_title="RR(t-τ)", zaxis_title="RR(t-2τ)",
                            uirevision="attractor_camera_live_v15514_sync"
                        ),
                        legend_title="Registro"
                    )
                    _render_live_attractor_3d(
                        _fig_sync, _anim_tracks3_v15514,
                        frame_duration_ms=_anim_speed_ms,
                        frame_cap=int(_anim_frames),
                        height=680
                    )
                    _attractor_video_export_ui(
                        _anim_tracks3_v15514,
                        key_prefix="attractor_video_sync_v15515"
                    )
                    st.markdown("---")
                        ...
                        for _ci, _pid in enumerate(_attr_pids):
                            ...
                            with _cols[_ci]:
                                ...
                                if _m_rec>=3:
                                    ...
                                    _fi.update_layout(height=500,scene=dict(xaxis_title="RR(t)",yaxis_title="RR(t-τ)",zaxis_title="RR(t-2τ)",uirevision="attractor_camera_live_v15514"),showlegend=False,margin=dict(l=0,r=0,t=20,b=0))
                                else:
                                ...
                                if _animate_attr and _m_rec >= 3:
                                    _render_live_attractor_3d(
                                        _fi, [(_short_record_label(_rec, 28), _show)],
                                        frame_duration_ms=_anim_speed_ms,
                                        frame_cap=int(_anim_frames),
                                        height=500
                                    )
                                    _attractor_video_export_ui(
                                        [(_short_record_label(_rec, 28), _show)],
                                        key_prefix=f"attractor_video_ind_multi_{hashlib.md5((str(_pid)+'|'+str(_rec)).encode()).hexdigest()[:12]}_v15516"
                                    )
                                else:
                                    if _animate_attr:
                                        _fi = _configure_attractor_animation(_fi, _show, dims=2, frame_duration_ms=_anim_speed_ms, frame_cap=int(_anim_frames))
                                    st.plotly_chart(_fi,use_container_width=True,key=f"attr_pat_{hashlib.md5((str(_pid)+'|'+str(_rec)).encode()).hexdigest()[:12]}_v15514")
                else:
                        ...
                        for _j, _rec in enumerate(_attr_records_visible[_i:_i+_ncols]):
                            with _cols[_j]:
                                ...
                                if _m_rec>=3:
                                    ...
                                    _fi.update_layout(height=500 if _ncols == 1 else 430,scene=dict(xaxis_title="RR(t)",yaxis_title="RR(t-τ)",zaxis_title="RR(t-2τ)",uirevision="attractor_camera_live_v15514"),showlegend=False)
                                else:
                                ...
                                if _animate_attr and _m_rec >= 3:
                                    _render_live_attractor_3d(
                                        ...
                                        frame_duration_ms=_anim_speed_ms,
                                        frame_cap=int(_anim_frames),
                                        height=500 if _ncols == 1 else 430
                                    )
                                    _attractor_video_export_ui(
                                        [(_short_record_label(_rec, 28), _show)],
                                        key_prefix=f"attractor_video_ind_{hashlib.md5(str(_rec).encode()).hexdigest()[:10]}_v15516"
                                    )
                                else:
                                    if _animate_attr:
                                        _fi = _configure_attractor_animation(_fi, _show, dims=2, frame_duration_ms=_anim_speed_ms, frame_cap=int(_anim_frames))
                                    st.plotly_chart(_fi,use_container_width=True,key=f"attr_ind_{hashlib.md5(str(_rec).encode()).hexdigest()[:10]}_v15514")
...
with tab9:
    st.subheader("Índices fisiológicos · contextualización etaria · Modo ultraligero")
    st.info(
    ...
    _profile_maps_v15535 = _load_all_reference_profiles_v15535(str(LONGITUDINAL_DB_PATH))
    _ctx_available_v15535 = bool(_profile_maps_v15535[0] or _profile_maps_v15535[1])

    _idx_mode_options_v15534 = ["Contextualizado por edad/sexo", "Bruto / histórico"]
    _idx_mode_default_v15534 = 0 if _ctx_available_v15535 else 1
    _idx_mode_v15534 = st.radio(
        "Capa de índices",
        _idx_mode_options_v15534,
        index=_idx_mode_default_v15534,
        horizontal=True,
        key="physio_indices_mode_v15534"
    )

    if _idx_mode_v15534 == "Contextualizado por edad/sexo":
        st.caption(
            "Modo ultraligero: la segunda capa se cachea también como DataFrame completo. "
            "Si los registros y perfiles no cambian, no se reconstruye fila por fila en cada interacción."
        )
        st.info(
            "En la capa contextual, **Regulación lenta** representa la intensidad/carga del componente VLF/lento, "
            "no una escala de 'peor→mejor'. Un valor alto indica mayor expresión lenta. "
            "El **Equilibrio lento** se muestra aparte y valora si VLF, DFAα2 y Hurst permanecen en una zona funcional."
        )
        _indices_ctx_full_v15535 = _contextualize_indices_bulk_v15539(indices_df, _profile_maps_v15535)
        _indices_active_v15534 = _indices_mode_frame_v15534(_indices_ctx_full_v15535, _idx_mode_v15534)
    else:
        st.caption(
            "Vista histórica original: no se ejecuta ningún cálculo contextual."
        )
        _indices_ctx_full_v15535 = pd.DataFrame()
        _indices_active_v15534 = indices_df
    if _indices_active_v15534.empty:
        st.info("No hay datos suficientes para calcular índices.")
    else:
        if _idx_mode_v15534 == "Contextualizado por edad/sexo":
            with st.expander("Comparar índices brutos vs contextualizados", expanded=False):
                _audit_cols_v15534 = ['Registro','Fase','IDX_CTX_Edad','IDX_CTX_Sexo','IDX_CTX_GrupoEdad']
                for _base_v15534 in ['IDX_Vagal','IDX_Amplitud','IDX_Complejidad','IDX_Rigidez','IDX_Adaptabilidad','IDX_Regulacion_Lenta']:
                    _audit_cols_v15534 += [_base_v15534, f"{_base_v15534}_CTX"]
                if 'IDX_Equilibrio_Regulacion_Lenta_CTX' in _indices_ctx_full_v15535.columns:
                    _audit_cols_v15534 += ['IDX_Equilibrio_Regulacion_Lenta_CTX']
                st.dataframe(
                    _indices_ctx_full_v15535[[c for c in _audit_cols_v15534 if c in _indices_ctx_full_v15535.columns]],
                    use_container_width=True,
                    hide_index=True
                )
        view_mode = st.radio(
        ...
        show_cols=['Registro','Fase','Paciente_ID','IDX_Vagal','IDX_Amplitud','IDX_Complejidad','IDX_Rigidez','IDX_Adaptabilidad','IDX_Regulacion_Lenta','IDX_Equilibrio_Regulacion_Lenta_CTX','Perfil_autonomico']
        if view_mode == "Registro seleccionado":
            st.plotly_chart(physiological_indices_figure(_indices_active_v15534, selected_record), use_container_width=True, key=f"physio_indices_{selected_record}")
            st.dataframe(_indices_active_v15534[[c for c in show_cols if c in _indices_active_v15534.columns]], use_container_width=True)
        elif view_mode == "Comparación cronológica actual":
            available_phases = [str(x) for x in _indices_active_v15534['Fase'].dropna().unique().tolist()]
            ...
            _idx_current_n = _record_count(_indices_active_v15534)
            _icp, _icnp, _ic0, _ic1 = _record_page_controls(_idx_current_n, "indices_current_v1543", page_size=24, label="registros")
            _indices_visible_v1543 = _dataframe_record_slice(_indices_active_v15534, _ic0, _ic1)
            ...
            if _idx_mode_v15534 == "Contextualizado por edad/sexo":
                st.caption(
                    "Los valores mostrados son la capa contextualizada. Los índices brutos siguen almacenados sin modificación."
                )
        else:
            if history_df_v140.empty:
                ...
                if _idx_mode_v15534 == "Contextualizado por edad/sexo":
                    hdf=_contextualize_indices_bulk_v15539(hdf, _profile_maps_v15535)
                    hdf=_indices_mode_frame_v15534(hdf, _idx_mode_v15534)
...
with tab10:
    st.subheader("Control autonómico · Diana V/S +  fuzzy adaptativo + desacoplamiento fisiológico · Modo ultraligero")
    st.info(
        "**Principio de esta versión:** el estado transversal actual y la evolución longitudinal se calculan y muestran por separado. "
        "Los Z-scores no pueden cambiar la etiqueta principal del estado fisiológico instantáneo."
    )
    st.caption(
        "Marco heurístico fisiológico y explicable: no es un diagnóstico. LF y LF/HF no se consideran medidas puras de actividad simpática. "
        "DFA, Hurst, Lyapunov, entropías y métricas de red se interpretan por convergencia y contexto."
    )

    with st.expander("Matriz definitiva de variables, pesos y función fisiológica", expanded=False):
        _matrix_auto_v15521 = pd.DataFrame(AUTONOMIC_MATRIX_V1554)
        st.dataframe(_matrix_auto_v15521, use_container_width=True, hide_index=True)
        st.caption(
            "Los pesos de la matriz construyen los vectores. La etiqueta instantánea utiliza exclusivamente Nivel_actual V/S/L/C/R. "
            "El plano longitudinal utiliza exclusivamente desplazamientos individuales respecto a sesiones previas."
        )

    # v15.5.39 · Cache de evaluación autonómica completa.
    # Streamlit ejecuta el contenido de todos los tabs en cada rerun; por eso
    # evitamos recalcular todos los registros si métricas/perfiles no han cambiado.
    _auto_profiles_v15539 = _load_all_reference_profiles_v15535(str(LONGITUDINAL_DB_PATH))
    _auto_history_v15539 = history_df_v140 if 'history_df_v140' in globals() else None
    _auto_sig_v15539 = _autonomic_cache_signature_v15539(
        long_df, indices_df, _auto_history_v15539, _auto_profiles_v15539,
        record_data=record_data
    )
    _auto_cache_v15539 = st.session_state.get("_auto_table_cache_v15539", {})
    if _auto_cache_v15539.get("signature") == _auto_sig_v15539:
        _auto_table_v15521 = _auto_cache_v15539.get("table", pd.DataFrame()).copy()
        _auto_source_v15521 = _auto_cache_v15539.get("source", pd.DataFrame()).copy()
    else:
        _auto_table_v15521, _auto_source_v15521 = build_autonomic_control_table(
            long_df, indices_df, record_data=record_data,
            reference_history=_auto_history_v15539
        )
        st.session_state["_auto_table_cache_v15539"] = {
            "signature": _auto_sig_v15539,
            "table": _auto_table_v15521.copy(),
            "source": _auto_source_v15521.copy(),
        }

    if _auto_table_v15521.empty:
        st.info("No hay métricas suficientes para evaluar el Control Autonómico en los registros/fases actuales.")
    else:
        # ============================================================
        # v15.5.32 · Perfil edad/sexo a nivel de PACIENTE, no de registro
        # ============================================================
        _auto_table_v15532 = _auto_table_v15521.copy()
        _auto_table_v15532['_Paciente_canonico_v15532'] = _auto_table_v15532.apply(
            lambda _r: infer_patient_id(
                str(_r.get('Paciente_ID', '') or _r.get('Registro', ''))
            ),
            axis=1
        )
        _patient_options_v15532 = sorted(
            [x for x in _auto_table_v15532['_Paciente_canonico_v15532'].dropna().astype(str).unique().tolist() if x]
        )

        st.markdown("### Datos de referencia del paciente · normalización transversal")
        if _patient_options_v15532:
            _profile_patient_v15532 = st.selectbox(
                "Paciente al que asignar edad y sexo",
                _patient_options_v15532,
                format_func=lambda x: _patient_display_name(x) if '_patient_display_name' in globals() else str(x),
                key="autonomic_profile_patient_v15532"
            )
            _matching_rows_v15532 = _auto_table_v15532[
                _auto_table_v15532['_Paciente_canonico_v15532'] == _profile_patient_v15532
            ]
            _matching_records_v15532 = _matching_rows_v15532['Registro'].astype(str).drop_duplicates().tolist()

            _profile_saved_v15532 = _load_patient_reference_profile_v15528(_profile_patient_v15532)

            with st.container(border=True):
                st.markdown(
                    f"**Perfil único del paciente:** `{_profile_patient_v15532}` · "
                    f"se aplicará automáticamente a **{len(_matching_records_v15532)} registro(s)**."
                )
                with st.expander("Ver registros que heredarán este perfil", expanded=False):
                    for _rr_v15532 in _matching_records_v15532:
                        st.write(f"• {_rr_v15532}")

                _pc1_v15532, _pc2_v15532, _pc3_v15532 = st.columns([1,1,1])
                with _pc1_v15532:
                    _saved_age_v15532 = _profile_saved_v15532.get('age_years')
                    _age_default_v15532 = int(_saved_age_v15532) if _saved_age_v15532 is not None else 50
                    _age_input_v15532 = st.number_input(
                        "Edad",
                        min_value=18, max_value=110,
                        value=_age_default_v15532, step=1,
                        key=f"autonomic_age_v15532_{_profile_patient_v15532}"
                    )
                with _pc2_v15532:
                    _sex_options_v15532 = ["No especificado", "Hombre", "Mujer"]
                    _sex_saved_v15532 = _profile_saved_v15532.get('sex', 'No especificado')
                    _sex_idx_v15532 = _sex_options_v15532.index(_sex_saved_v15532) if _sex_saved_v15532 in _sex_options_v15532 else 0
                    _sex_input_v15532 = st.selectbox(
                        "Sexo",
                        _sex_options_v15532,
                        index=_sex_idx_v15532,
                        key=f"autonomic_sex_v15532_{_profile_patient_v15532}"
                    )
                with _pc3_v15532:
                    st.write("")
                    st.write("")
                    if st.button(
                        "Guardar perfil para todos sus registros",
                        key=f"save_autonomic_profile_v15532_{_profile_patient_v15532}",
                        use_container_width=True
                    ):
                        _save_patient_reference_profile_v15528(
                            _profile_patient_v15532,
                            _age_input_v15532,
                            _sex_input_v15532
                        )
                        try:
                            _cloud_checkpoint_v1540("patient_reference_profile_v15528", force=True)
                        except Exception:
                            pass
                        # Limpiar cachés que puedan conservar evaluaciones anteriores.
                        try:
                            _load_all_reference_profiles_v15535.clear()
                            _contextualize_indices_bulk_cached_v15539.clear()
                        except Exception:
                            pass
                        st.session_state.pop("_auto_table_cache_v15539", None)
                        st.success(
                            f"Perfil guardado para {_profile_patient_v15532}. "
                            f"Se aplicará a sus {len(_matching_records_v15532)} registro(s)."
                        )
                        st.rerun()

                _grp_show_v15532 = _age_group_v15528(_age_input_v15532)
                _saved_txt_v15532 = (
                    f"Guardado: {_profile_saved_v15532.get('age_years')} años · {_profile_saved_v15532.get('sex')}"
                    if _profile_saved_v15532.get('age_years') is not None
                    else "Perfil aún no guardado"
                )
                st.caption(
                    f"{_saved_txt_v15532}. Referencia mostrada: {_sex_input_v15532} · grupo {_grp_show_v15532}. "
                    "Edad/sexo sólo contextualizan el plano actual; el longitudinal sigue siendo 100% intraindividual."
                )

                st.markdown("#### Cambiar edad/sexo sólo en uno o varios registros")
                st.caption(
                    "Útil cuando el paciente cumple años durante el seguimiento o necesitas corregir "
                    "una edad/sexo en un tramo concreto. Los registros no seleccionados mantienen el perfil general."
                )
                _override_records_v15533 = st.multiselect(
                    "Registros a modificar",
                    _matching_records_v15532,
                    key=f"autonomic_profile_records_v15533_{_profile_patient_v15532}"
                )
                if _override_records_v15533:
                    _ov1_v15533, _ov2_v15533, _ov3_v15533 = st.columns([1,1,1])
                    with _ov1_v15533:
                        _ov_age_v15533 = st.number_input(
                            "Edad para registros seleccionados",
                            min_value=18, max_value=110,
                            value=int(_age_input_v15532), step=1,
                            key=f"autonomic_override_age_v15533_{_profile_patient_v15532}"
                        )
                    with _ov2_v15533:
                        _ov_sex_v15533 = st.selectbox(
                            "Sexo para registros seleccionados",
                            ["No especificado", "Hombre", "Mujer"],
                            index=["No especificado", "Hombre", "Mujer"].index(_sex_input_v15532),
                            key=f"autonomic_override_sex_v15533_{_profile_patient_v15532}"
                        )
                    with _ov3_v15533:
                        st.write("")
                        st.write("")
                        if st.button(
                            "Aplicar a registros seleccionados",
                            key=f"save_record_profile_v15533_{_profile_patient_v15532}",
                            use_container_width=True
                        ):
                            _save_record_reference_profile_v15533(
                                _profile_patient_v15532,
                                _override_records_v15533,
                                _ov_age_v15533,
                                _ov_sex_v15533
                            )
                            try:
                                _cloud_checkpoint_v1540("record_reference_profile_v15533", force=True)
                            except Exception:
                                pass
                            try:
                                _load_all_reference_profiles_v15535.clear()
                            ...
                            st.session_state.pop("_auto_table_cache_v15539", None)
                            st.success(
                                f"Edad/sexo aplicados a {len(_override_records_v15533)} registro(s) seleccionados."
                            )
                            st.rerun()

        else:
            st.info("No se ha podido identificar ningún paciente en los registros visibles.")

        _auto_labels_v15521 = []
        for _i, _r in _auto_table_v15521.iterrows():
            _dt = pd.to_datetime(_r.get('Fecha_hora'), errors='coerce')
            _ds = _dt.strftime('%d/%m/%Y %H:%M:%S') if pd.notna(_dt) else ''
            _auto_labels_v15521.append(
                f"{_r.get('Registro','')} · {_r.get('Fase','')}" + (f" · {_ds}" if _ds else '')
            )

        _auto_sel_idx_v15521 = st.selectbox(
            "Registro/fase para interpretar",
            range(len(_auto_table_v15521)),
            format_func=lambda i: _auto_labels_v15521[int(i)],
            key="autonomic_control_record_v15521"
        )
        _ar = _auto_table_v15521.iloc[int(_auto_sel_idx_v15521)]
        _selected_patient_v15532 = infer_patient_id(
            str(_ar.get('Paciente_ID', '') or _ar.get('Registro', ''))
        )
        _selected_profile_v15532 = _resolve_reference_profile_v15533(
            _selected_patient_v15532, str(_ar.get('Registro',''))
        )
        if _selected_profile_v15532.get('age_years') is not None:
            _src_profile_txt_v15533 = (
                "override específico del registro"
                if _selected_profile_v15532.get('source') == "registro"
                else "perfil general del paciente"
            )
            st.caption(
                f"Perfil aplicado a este registro: "
                f"{_selected_profile_v15532.get('age_years')} años · "
                f"{_selected_profile_v15532.get('sex', 'No especificado')} · "
                f"{_src_profile_txt_v15533}."
            )
        else:
            st.warning(
                f"El paciente `{_selected_patient_v15532}` aún no tiene edad/sexo guardados. "
                "Guárdalos una sola vez en el bloque superior; se aplicarán a todos sus registros."
            )

        # ============================================================
        # ENCABEZADO DUAL PRINCIPAL
        # ============================================================
        _plain_v15529 = _autonomic_plain_language_v15529(_ar)

        st.markdown("## Modo de control autonómico actual")
        with st.container(border=True):
            st.markdown(
                f"### **{_plain_v15529['modo_actual']} / {_plain_v15529['predominio_actual']}**"
            )
            st.write(
                "Este es el resultado principal de la sesión actual. "
                "Se calcula sólo con los datos actuales contextualizados por edad y sexo."
            )

            _v_now_v15537 = pd.to_numeric(_ar.get('Vector_V_Actual', np.nan), errors='coerce')
            _s_now_v15537 = pd.to_numeric(_ar.get('Vector_S_Actual', np.nan), errors='coerce')
            _vs_delta_v15537 = (
                float(_v_now_v15537 - _s_now_v15537)
                if np.isfinite(_v_now_v15537) and np.isfinite(_s_now_v15537)
                else np.nan
            )
            _vs1_v15537, _vs2_v15537, _vs3_v15537 = st.columns(3)
            with _vs1_v15537:
                st.metric(
                    "V · Vagal actual",
                    f"{float(_v_now_v15537):+.2f}" if np.isfinite(_v_now_v15537) else "N/D"
                )
            with _vs2_v15537:
                st.metric(
                    "S · Alerta/Barorreflejo actual",
                    f"{float(_s_now_v15537):+.2f}" if np.isfinite(_s_now_v15537) else "N/D"
                )
            with _vs3_v15537:
                st.metric(
                    "Diferencia V − S",
                    f"{_vs_delta_v15537:+.2f}" if np.isfinite(_vs_delta_v15537) else "N/D"
                )
            if np.isfinite(_vs_delta_v15537):
                if _vs_delta_v15537 >= 0.20:
                    _vs_explain_v15537 = (
                        "V supera claramente a S: el predominio final se mantiene Vagal. "
                        "El espectro puede apoyar o señalar discordancia, pero no invertir esta decisión."
                    )
                elif _vs_delta_v15537 <= -0.20:
                    _vs_explain_v15537 = (
                        "S supera claramente a V: el predominio final se mantiene Alerta/Barorreflejo. "
                        "El espectro puede apoyar o señalar discordancia, pero no invertir esta decisión."
                    )
                else:
                    _vs_explain_v15537 = (
                        "V y S están próximos: en este caso HF_DOM, LF_DOM y LF/HF actuales "
                        "pueden decidir el predominio."
                    )
                st.caption(_vs_explain_v15537)
                _pred_expl_v15538 = str(_ar.get('Explicacion_predominio_actual', '') or '')
                if _pred_expl_v15538:
                    st.caption("Trazabilidad: " + _pred_expl_v15538)

            if bool(_ar.get('Discordancia_espectral_actual', _ar.get('discordancia_espectral', False))):
                st.warning(
                    "Existe discordancia entre V/S y el espectro actual. "
                    "La etiqueta principal conserva V/S porque la diferencia es clara; "
                    "la discordancia queda visible para interpretación clínica."
                )

            st.markdown("### Mapa visual V/S")
            _traj_mode_v15543 = st.radio(
                "Trayectoria mostrada",
                ["Misma fase del paciente", "Todos los registros/fases visibles", "Sólo estado actual"],
                horizontal=True,
                key="autonomic_dual_traj_mode_v15543"
            )

            _hist_v_v15543, _hist_s_v15543, _hist_lab_v15543 = [], [], []
            if _traj_mode_v15543 != "Sólo estado actual":
                _hist_df_v15543 = _auto_table_v15521.copy()
                if 'Paciente_ID' in _hist_df_v15543.columns:
                    _hist_df_v15543 = _hist_df_v15543[
                        _hist_df_v15543['Paciente_ID'].astype(str).map(normalize_patient_id)
                        == normalize_patient_id(str(_ar.get('Paciente_ID','')))
                    ]
                else:
                    _hist_df_v15543 = _hist_df_v15543[
                        _hist_df_v15543['Registro'].astype(str).map(infer_patient_id).map(normalize_patient_id)
                        == normalize_patient_id(str(_selected_patient_v15532))
                    ]
                if _traj_mode_v15543 == "Misma fase del paciente" and 'Fase' in _hist_df_v15543.columns:
                    _hist_df_v15543 = _hist_df_v15543[
                        _hist_df_v15543['Fase'].astype(str) == str(_ar.get('Fase',''))
                    ]
                if 'Fecha_hora' in _hist_df_v15543.columns:
                    _hist_df_v15543 = _hist_df_v15543.sort_values(
                        ['Fecha_hora','Registro'], na_position='last'
                    )
                for _, _hr_v15543 in _hist_df_v15543.iterrows():
                    _vv = pd.to_numeric(_hr_v15543.get('Vector_V_Actual', np.nan), errors='coerce')
                    _ss = pd.to_numeric(_hr_v15543.get('Vector_S_Actual', np.nan), errors='coerce')
                    if np.isfinite(_vv) and np.isfinite(_ss):
                        _hist_v_v15543.append(float(_vv))
                        _hist_s_v15543.append(float(_ss))
                        _hdt = pd.to_datetime(_hr_v15543.get('Fecha_hora'), errors='coerce')
                        _hds = _hdt.strftime('%d/%m/%Y %H:%M') if pd.notna(_hdt) else str(_hr_v15543.get('Registro',''))
                        _hist_lab_v15543.append(f"{_hds} · {_hr_v15543.get('Fase','')}")

            _gray_v15543 = pd.to_numeric(_ar.get('Umbral_gris_actual', 0.20), errors='coerce')
            _incoh_v15543 = bool(_ar.get('Incoherencia_detectada', False)) or (
                "incoherente" in str(_ar.get('Topologia_instantanea','')).lower()
            )
            _dual_fig_v15543 = _autonomic_dual_dashboard_v15543(
                V_abs=_v_now_v15537,
                S_abs=_s_now_v15537,
                history_V=_hist_v_v15543,
                history_S=_hist_s_v15543,
                history_labels=_hist_lab_v15543,
                gray=float(_gray_v15543) if np.isfinite(_gray_v15543) else 0.20,
                topology=str(_ar.get('Topologia_instantanea','')),
                predominance=str(_ar.get('Predominio_instantaneo','')),
                incoherent=_incoh_v15543,
                animate=(_traj_mode_v15543 != "Sólo estado actual")
            )
            st.plotly_chart(
                _dual_fig_v15543,
                use_container_width=True,
                key=f"autonomic_dual_dashboard_v15543_{int(_auto_sel_idx_v15521)}"
            )
            st.caption(
                "La diana usa S = Alerta/Barorreflejo, no actividad simpática pura. "
                "La franja central se adapta al umbral fuzzy de calidad del registro actual. "
                "Las bandas discontinuas junto a los ejes representan zonas compatibles con "
                "desacoplamiento fisiológico unilateral; la incoherencia metodológica se informa aparte."
            )

            _sum1_v15529, _sum2_v15529 = st.columns(2)
            with _sum1_v15529:
                st.markdown("#### Amplitud / reserva actual")
                st.markdown(f"**{_plain_v15529['reserva_actual']}**")
            with _sum2_v15529:
                st.markdown("#### Evolución respecto a registros anteriores")
                st.markdown(f"**{_plain_v15529['evolucion_simple']}**")

            st.write(_plain_v15529['evolucion_nota_simple'])
            st.caption(
                f"{_plain_v15529['magnitud_simple']} · "
                f"{_plain_v15529['persistencia_simple']}"
            )

        st.markdown("### Evolución personal")
        st.caption(
            "Esta sección compara el registro con el propio histórico del paciente. "
            "Puede mejorar la reserva/capacidad reguladora aunque el predominio autonómico ACTUAL siga siendo vagal o alerta/barorreflejo."
        )
        with st.container(border=True):
            st.markdown(f"**{_plain_v15529['evolucion_simple']}**")
            st.write(_plain_v15529['evolucion_nota_simple'])
            st.markdown(
                f"**Magnitud del cambio:** {_plain_v15529['magnitud_simple']}  \n"
                f"**Confirmación temporal:** {_plain_v15529['persistencia_simple']}  \n"
                f"**Cambio de reserva personal:** {_plain_v15529['reserva_long_simple']}"
            )

        with st.expander("Análisis integrado 70/30", expanded=False):
            _autonomic_wrap_card_v15526(
                "Estado integrado",
                str(_ar.get('Estado_integrado_70_30', 'N/D')),
                (
                    "Síntesis secundaria: 70% del estado actual + 30% de la evolución individual. "
                    "No sustituye al modo de control actual."
                )
            )
            _eff_df_v15529 = pd.DataFrame({
                'Vector': [
                    'Vagal (V)','Alerta/barorreflejo (S)','Regulación lenta (L)',
                    'Complejidad/movilidad (C)','Reserva/amplitud (R)'
                ],
                'Actual': [
                    _ar.get('Vector_V_Actual', np.nan), _ar.get('Vector_S_Actual', np.nan),
                    _ar.get('Vector_L_Actual', np.nan), _ar.get('Vector_C_Actual', np.nan),
                    _ar.get('Vector_R_Actual', np.nan)
                ],
                'Longitudinal_Z': [
                    _ar.get('Vector_V_Z', np.nan), _ar.get('Vector_S_Z', np.nan),
                    _ar.get('Vector_L_Z', np.nan), _ar.get('Vector_C_Z', np.nan),
                    _ar.get('Vector_R_Z', np.nan)
                ],
                'Integrado_70_30': [
                    _ar.get('Vector_V_Efectivo', np.nan), _ar.get('Vector_S_Efectivo', np.nan),
                    _ar.get('Vector_L_Efectivo', np.nan), _ar.get('Vector_C_Efectivo', np.nan),
                    _ar.get('Vector_R_Efectivo', np.nan)
                ],
            })
            st.dataframe(_eff_df_v15529, use_container_width=True, hide_index=True)

        with st.expander("Ver confianza y calidad del registro", expanded=False):
            _q1, _q2 = st.columns(2)
            with _q1:
                st.metric(
                    "Confianza de la clasificación actual",
                    f"{float(_ar.get('Confianza_clasificacion_pct', np.nan)):.0f}%"
                )
            with _q2:
                st.metric(
                    "Calidad/contexto Q",
                    f"{100*float(_ar.get('Calidad_Q', np.nan)):.0f}%"
                )
            st.caption(
                "Estos valores ayudan a juzgar la fiabilidad del resultado, "
                "pero no son el modo de control autonómico."
            )

        with st.expander("Análisis avanzado · vectores, espectro y Z-scores", expanded=False):
            # ============================================================
            # PLANO ACTUAL / ABSOLUTO
            # ============================================================
            st.markdown("### Estado fisiológico actual · vectores absolutos")
            st.caption(
                "La etiqueta instantánea se calcula sólo con los niveles actuales, contextualizados por edad y sexo. "
                "La mezcla 70/30 se recupera como un tercer plano integrado y separado; no modifica la etiqueta actual ni la longitudinal. "
                "V/S/L/C/R actuales usan referencias por edad cuando existen; S mantiene la interpretación LF/barorrefleja y no simpática pura; "
                "IDX_Rigidez permanece como descriptor independiente."
            )

            _abs_df_v15521 = pd.DataFrame({
                'Vector': [
                    'Vagal (V)', 'Alerta/barorreflejo (S)', 'Regulación lenta (L)',
                    'Complejidad/movilidad (C)', 'Reserva/amplitud (R)'
                ],
            ...
            st.dataframe(
                ...
                hide_index=True,
                column_config={
                    "Nivel_actual": st.column_config.NumberColumn(
                        "Nivel actual", format="%.2f",
                        help="100% transversal/instantáneo; no contiene información longitudinal."
                    )
                },
            )

            with st.expander("Desglose de normalización por edad/sexo", expanded=False):
                st.caption(
                    "Esta tabla explica qué valores actuales elevan o reducen los vectores transversales. "
                    "No utiliza sesiones previas."
                )
                try:
                    _detail_norm_v15528 = json.loads(_ar.get('Detalle_normalizacion_edad', '[]') or '[]')
                    if _detail_norm_v15528:
                        st.dataframe(pd.DataFrame(_detail_norm_v15528), use_container_width=True, hide_index=True)
                    else:
                        st.info("Guarda edad/sexo del paciente para activar la normalización contextual.")
                except Exception:
                    st.info("No se pudo reconstruir el detalle contextual para este registro.")

            _fig_now_v15521 = go.Figure(go.Bar(
                x=_abs_df_v15521['Vector'], y=_abs_df_v15521['Nivel_actual'],
                name='Nivel actual'
            ))
            _fig_now_v15521.add_hline(y=0, line_dash='dash')
            _fig_now_v15521.update_layout(
                height=360, yaxis_title='Nivel transversal normalizado',
                xaxis_title='', title='Plano actual V/S/L/C/R'
            )
            st.plotly_chart(_fig_now_v15521, use_container_width=True, key='autonomic_vectors_current_v15521')

            _rig_now_v15523 = _ar.get('Rigidez_actual_independiente', np.nan)
            _r1c1, _r1c2 = st.columns(2)
            with _r1c1:
                _autonomic_wrap_card_v15526(
                    "Topología actual",
                    str(_ar.get('Topologia_instantanea', 'N/D')),
                    "Familia transversal: Reciprocidad, Coactivación, Coinhibición o Desacoplamiento."
                )
            with _r1c2:
                _autonomic_wrap_card_v15526(
                    "Predominio actual",
                    str(_ar.get('Predominio_instantaneo', 'N/D')),
                    str(_ar.get('Apoyo_espectral_transversal', ''))
                )
            _r2c1, _r2c2 = st.columns(2)
            with _r2c1:
                _autonomic_wrap_card_v15526(
                    "Reserva actual absoluta",
                    str(_ar.get('Reserva_instantanea', 'N/D')),
                    f"Histórico separado: {_ar.get('Reserva_longitudinal_relativa', 'N/D')}."
                )
            with _r2c2:
                _autonomic_wrap_card_v15526(
                    "Rigidez dinámica actual",
                    f"{float(_rig_now_v15523):.1f}/100" if np.isfinite(_rig_now_v15523) else "N/D",
                    "Descriptor independiente; ya no sustituye al vector S."
                )

            st.caption(
                "v15.5.26: cuatro familias topológicas; los casos cercanos a frontera se marcan como limítrofes. "
                "El patrón oscilatorio se informa aparte para no confundir potencia absoluta con dominancia porcentual."
            )
            st.info(
                f"**Reserva longitudinal relativa:** {_ar.get('Reserva_longitudinal_relativa', 'N/D')}. "
                "No modifica la reserva absoluta de la sesión actual."
            )

            st.markdown("### Patrón oscilatorio · potencia y distribución")
            _osc1_v15526, _osc2_v15526 = st.columns(2)
            with _osc1_v15526:
                _autonomic_wrap_card_v15526(
                    "Composición espectral actual",
                    str(_ar.get('Patron_oscilatorio_actual', 'N/D')),
                    str(_ar.get('Nota_patron_oscilatorio_actual', ''))
                )
            with _osc2_v15526:
                _autonomic_wrap_card_v15526(
                    "Cambio de potencia respecto al histórico",
                    str(_ar.get('Patron_oscilatorio_longitudinal', 'N/D')),
                    str(_ar.get('Nota_patron_oscilatorio_longitudinal', ''))
                )

            _sp1_v15524, _sp2_v15524, _sp3_v15524 = st.columns(3)
            _sp1_v15524.metric("HF_DOM actual", f"{float(_ar.get('HF_DOM_actual_control', np.nan)):.1f}%" if np.isfinite(_ar.get('HF_DOM_actual_control', np.nan)) else "N/D")
            _sp2_v15524.metric("LF_DOM actual", f"{float(_ar.get('LF_DOM_actual_control', np.nan)):.1f}%" if np.isfinite(_ar.get('LF_DOM_actual_control', np.nan)) else "N/D")
            _sp3_v15524.metric("LF/HF actual", f"{float(_ar.get('LF_HF_actual_control', np.nan)):.2f}" if np.isfinite(_ar.get('LF_HF_actual_control', np.nan)) else "N/D")

            if bool(_ar.get('Discordancia_espectral_transversal', False)):
                st.warning(
                    f"**Discordancia transversal detectada.** V/S vectorial sugería "
                    f"`{_ar.get('Predominio_vectorial_pre_espectral', 'N/D')}`, pero la evidencia espectral actual es suficientemente fuerte para apoyar "
                    f"`{_ar.get('Predominio_instantaneo', 'N/D')}`. No se han usado Z-scores ni datos longitudinales."
                )
            else:
                st.info(
                    f"**Coherencia espectral transversal:** {_ar.get('Apoyo_espectral_transversal', 'N/D')}. "
                    f"Predominio vectorial previo: {_ar.get('Predominio_vectorial_pre_espectral', 'N/D')}."
                )

            st.markdown("### Explicación del estado actual")
            st.write(str(_ar.get('Interpretacion_fisiologica', '')))
            st.caption(str(_ar.get('Nota_convergencia_actual', '')))
            with st.expander("Cómo se decide la topología actual", expanded=False):
                st.write(
                    "La decisión ya no depende sólo de Vagal (V) y Alerta/barorreflejo (S). "
                    "V/S definen principalmente la familia topológica (70%) y L/C/R modulan el soporte (30%). "
                    "La pertenencia es continua (fuzzy) y la zona gris depende sólo de la calidad del registro actual."
                )
                st.caption(
                    "Importante: **L (regulación lenta) es un eje de intensidad**, no de calidad. "
                    "L≈0 indica rango lento preservado; L<0 menor expresión lenta; L>0 mayor carga/dominancia lenta. "
                    "La topología premia el equilibrio de L, no que L sea alto."
                )
                _l1_v15536, _l2_v15536, _l3_v15536 = st.columns(3)
                with _l1_v15536:
                    st.metric("Equilibrio lento", f"{100*float(_ar.get('Equilibrio_Lento_actual', np.nan)):.0f}%" if np.isfinite(_ar.get('Equilibrio_Lento_actual', np.nan)) else "N/D")
                with _l2_v15536:
                    st.metric("Carga lenta elevada", "Sí" if bool(_ar.get('Carga_Lenta_elevada_actual', False)) else "No")
                with _l3_v15536:
                    st.metric("Expresión lenta reducida", "Sí" if bool(_ar.get('Expresion_Lenta_reducida_actual', False)) else "No")
                _top_scores_v15530 = pd.DataFrame({
                    "Familia": [
                        "Coactivación","Reciprocidad","Coinhibición",
                        "Desacoplamiento fisiológico unilateral","Patrón complejo/incoherente",
                        "Patrón intermedio/no concluyente"
                    ],
                    "Permitida": [
                        bool(_ar.get('Coactivacion_permitida', False)),
                        bool(_ar.get('Reciprocidad_permitida', False)),
                        bool(_ar.get('Coinhibicion_permitida', False)),
                        bool(_ar.get('Desacoplamiento_permitido', False)),
                        bool(_ar.get('Incoherencia_detectada', False)),
                        True,
                    ],
                    "Score": [
                        _ar.get('Score_Coactivacion_actual', np.nan),
                        _ar.get('Score_Reciprocidad_actual', np.nan),
                        _ar.get('Score_Coinhibicion_actual', np.nan),
                        _ar.get('Score_Desacoplamiento_actual', np.nan),
                        _ar.get('Score_Incoherencia_actual', np.nan),
                        _ar.get('Score_Intermedio_actual', np.nan),
                    ],
                    "Score_bruto": [
                        _ar.get('Score_Coactivacion_bruto_actual', np.nan),
                        _ar.get('Score_Reciprocidad_bruto_actual', np.nan),
                        _ar.get('Score_Coinhibicion_bruto_actual', np.nan),
                        _ar.get('Score_Desacoplamiento_bruto_actual', np.nan),
                        _ar.get('Score_Incoherencia_actual', np.nan),
                        _ar.get('Score_Intermedio_actual', np.nan),
                    ]
                }).sort_values("Score", ascending=False)
                st.dataframe(_top_scores_v15530, use_container_width=True, hide_index=True)
                st.metric(
                    "Apoyos para Coactivación con V/S preservados",
                    f"{int(_ar.get('Apoyos_coactivacion_preservada', 0))}/4"
                )
                _st1_v15540, _st2_v15540 = st.columns(2)
                with _st1_v15540:
                    st.metric("Estado V actual", str(_ar.get('Estado_V_actual', 'N/D')))
                with _st2_v15540:
                    st.metric("Estado S actual", str(_ar.get('Estado_S_actual', 'N/D')))
                st.caption(
                    "El score mayor determina la familia. Si los dos primeros son muy próximos, "
                    "la app la marca como limítrofe en vez de forzar una categoría."
                )
                st.markdown("#### Compuerta fisiológica de Coinhibición")
                _gate_cols_v15531 = st.columns(2)
                with _gate_cols_v15531[0]:
                    st.metric(
                        "Actividad multibanda actual",
                        "Sí" if bool(_ar.get('Actividad_multibanda_actual', False)) else "No"
                    )
                with _gate_cols_v15531[1]:
                    st.metric(
                        "Coinhibición permitida",
                        "Sí" if bool(_ar.get('Coinhibicion_permitida', False)) else "No"
                    )
                st.write(str(_ar.get('Motivo_compuerta_coinhibicion', '')))
                st.caption(
                    "Coinhibición sólo puede ganar si V, S, reserva/amplitud y complejidad están "
                    "convergentemente reducidos y no existe actividad multibanda actual conservada."
                )
                st.markdown("#### Compuerta fisiológica de Desacoplamiento")
                st.metric(
                    "Desacoplamiento permitido",
                    "Sí" if bool(_ar.get('Desacoplamiento_permitido', False)) else "No"
                )
                st.caption(
                    "La simple dispersión entre V/S/L/C/R ya no basta. "
                    "Desacoplamiento sólo compite si existe una contradicción fisiológica explícita "
                    "entre subsistemas o entre V/S y el espectro actual."
                )

            # ============================================================
            # PLANO LONGITUDINAL SECUNDARIO / COLAPSABLE
            # ============================================================
            with st.expander("Análisis técnico de evolución (Z-scores)", expanded=False):
                st.caption(
                    "Taxonomía longitudinal = 100% Z/Δ individual respecto al histórico previo. "
                    "Incluye estabilidad, ganancia/pérdida de reserva, desplazamientos V/S, coactivación, coinhibición, "
                    "desacoplamiento oscilatorio, flexibilidad adaptativa, fragmentación, rigidificación y regulación lenta. "
                    "No usa Nivel_actual ni mezcla 70/30."
                )
                _ref_n_v15521 = int(_ar.get('Referencia_n', 0) or 0)
                _ref_method_v15521 = str(_ar.get('Referencia_metodo', 'Sin referencia longitudinal previa'))

                if _ref_n_v15521 >= 2:
                    st.info(
                        f"**Referencia individual:** {_ref_method_v15521}. "
                        "Los Z individuales usan sólo sesiones ANTERIORES del mismo paciente y priorizan la misma fase."
                    )
                elif _ref_n_v15521 == 1:
                    st.warning(
                        "Sólo existe una sesión previa comparable: se muestra Δ normalizado, "
                        "pero no se emite clasificación de reorganización por Z individual."
                    )
                else:
                    st.warning("No existe referencia longitudinal previa suficiente.")

                _vec_z_v15521 = pd.DataFrame({
                    'Vector': [
                        'Vagal (V)', 'Alerta/barorreflejo (S)', 'Regulación lenta (L)',
                        'Complejidad/movilidad (C)', 'Reserva/amplitud (R)'
                    ],
                ...
                st.dataframe(_vec_z_v15521, use_container_width=True, hide_index=True)

                _fig_z_v15521 = go.Figure(go.Bar(x=_vec_z_v15521['Vector'], y=_vec_z_v15521['Z_ind']))
                _fig_z_v15521.add_hline(y=0, line_dash='dash')
                _fig_z_v15521.add_hline(y=0.8, line_dash='dot')
                _fig_z_v15521.add_hline(y=-0.8, line_dash='dot')
                _fig_z_v15521.update_layout(
                    height=360, yaxis_title='Z individual / Δ normalizado',
                    xaxis_title='', title='Desplazamiento longitudinal V/S/L/C/R'
                )
                st.plotly_chart(_fig_z_v15521, use_container_width=True, key='autonomic_vectors_z_v15521')

                st.markdown("#### Indicadores longitudinales de frecuencia y complejidad")
                _z_detail_v15521 = pd.DataFrame({
                    'Indicador': ['HF', 'LF', 'VLF', 'TOTAL', 'MSE', 'MDE', 'Complejidad C'],
                    'Z_ind': [
                        _ar.get('HF_consenso_Z', np.nan),
                        _ar.get('LF_consenso_Z', np.nan),
                        _ar.get('VLF_consenso_Z', np.nan),
                        _ar.get('TOTAL_consenso_Z', np.nan),
                        _ar.get('MSE_multiescala_Z', np.nan),
                        _ar.get('MDE_multiescala_Z', np.nan),
                        _ar.get('Vector_C_Z', np.nan),
                    ]
                })
                st.dataframe(_z_detail_v15521, use_container_width=True, hide_index=True)

                if bool(_ar.get('Reorganizacion_longitudinal', False)):
                    st.warning(
                        f"**{_ar.get('Tendencia_longitudinal', 'Reorganización longitudinal')}** · "
                        f"Magnitud: {_ar.get('Severidad_longitudinal', 'N/D')} · "
                        f"Confirmación temporal: {_ar.get('Persistencia_longitudinal', 'N/D')}"
                    )
                    st.info(str(_ar.get('Nota_longitudinal', '')))
                else:
                    st.info(
                        f"**Tendencia longitudinal:** {_ar.get('Tendencia_longitudinal', 'N/D')} · "
                        f"Magnitud: {_ar.get('Severidad_longitudinal', 'N/D')} · "
                        f"Confirmación temporal: {_ar.get('Persistencia_longitudinal', 'N/D')}. "
                        f"{_ar.get('Nota_longitudinal', '')}"
                    )

            # ============================================================
            # JUSTIFICACIÓN Y CRONOLOGÍA
            # ============================================================
            with st.expander("Justificación algorítmica completa", expanded=False):
                st.markdown("**Plano instantáneo**")
                st.write(str(_ar.get('Justificacion_instantanea', '')))
                st.markdown("**Plano longitudinal**")
                st.write(str(_ar.get('Justificacion_longitudinal', '')))
                st.caption(
                    "Separación estricta: Estado actual = 100% datos actuales; longitudinal = 100% Z/Δ histórico. "
                    "Vector_*_Efectivo 70/30 se recupera como tercer plano integrado; no altera el estado actual ni el longitudinal."
                )

        with st.expander("Historial cronológico completo", expanded=False):
            st.markdown("### Evolución cronológica del Control Autonómico")
            _show_cols_v15521 = [
                'Fecha_hora','Registro','Fase',
                'Edad_referencia','Sexo_referencia','Grupo_edad_referencia','Estado_instantaneo','Topologia_instantanea','Familia_topologica_instantanea','Calificador_topologico_instantaneo','Predominio_instantaneo','Reserva_instantanea','Reserva_longitudinal_relativa','Predominio_vectorial_pre_espectral','Apoyo_espectral_transversal','Discordancia_espectral_transversal','HF_DOM_actual_control','LF_DOM_actual_control','LF_HF_actual_control',
                'Tendencia_longitudinal','Severidad_longitudinal','Persistencia_longitudinal','Reorganizacion_longitudinal','Reorganizacion_desacoplamiento_oscilatorio',
                'Confianza_clasificacion_pct','Referencia_n','Referencia_metodo',
                'Vector_V_Actual','Vector_S_Actual','Vector_L_Actual','Vector_C_Actual','Vector_R_Actual','Rigidez_actual_independiente',
                'Vector_V_Z','Vector_S_Z','Vector_L_Z','Vector_C_Z','Vector_R_Z',
                'Vector_V_Efectivo','Vector_S_Efectivo','Vector_L_Efectivo','Vector_C_Efectivo','Vector_R_Efectivo',
                'Estado_integrado_70_30','Topologia_integrada_70_30','Predominio_integrado_70_30','Reserva_integrada_70_30',
                'Patron_oscilatorio_actual','Patron_oscilatorio_longitudinal',
                'HF_consenso_Z','LF_consenso_Z','VLF_consenso_Z','TOTAL_consenso_Z',
                'MSE_multiescala_Z','MDE_multiescala_Z','Calidad_Q'
            ]
            _show_auto_v15521 = _auto_table_v15521[
                [c for c in _show_cols_v15521 if c in _auto_table_v15521.columns]
            ].copy()
            _show_auto_v15521 = _show_auto_v15521.sort_values('Fecha_hora', na_position='last')
            st.dataframe(_show_auto_v15521, use_container_width=True, hide_index=True)

            st.download_button(
                "Descargar Control Autonómico CSV",
                _auto_table_v15521.to_csv(index=False).encode('utf-8'),
                file_name='control_autonomico_v15543.csv',
                mime='text/csv'
            )
...
with tab11:
    ...
    if not _patient_pool_v1529:
        ...
        # v15.5.8 · Conserva narrativa superior v15.5.7: Presente → Tendencia → Probabilidad futura.
        # Esta capa es exclusivamente de presentación: no modifica Nivel 1, XGBoost, calibración ni persistencia.
        _horizon_preview_v1557 = int(st.session_state.get('v15412_horizon_days', 7))
        _phase_options_preview_v1557 = []
        if not _patient_history_v1529.empty and 'Fase' in _patient_history_v1529.columns:
            _phase_options_preview_v1557 = [str(x) for x in _patient_history_v1529['Fase'].dropna().astype(str).unique().tolist()]
        _phase_preview_v1557 = st.session_state.get(
            'v15412_probability_phase',
            (_phase_options_preview_v1557[0] if _phase_options_preview_v1557 else 'Basal')
        )
        if _phase_preview_v1557 not in (_phase_options_preview_v1557 or ['Basal']):
            _phase_preview_v1557 = (_phase_options_preview_v1557[0] if _phase_options_preview_v1557 else 'Basal')

        _pred_preview_v1557 = {}
        _summary_preview_v1557 = pd.DataFrame()
        if not _patient_history_v1529.empty:
            try:
                _pred_preview_v1557, _summary_preview_v1557, _ = predict_v15_all_phases(
                    _patient_history_v1529, pa, horizon_days=float(_horizon_preview_v1557)
                )
            except Exception:
                _pred_preview_v1557, _summary_preview_v1557 = {}, pd.DataFrame()

        _phase_row_preview_v1557 = None
        if not _summary_preview_v1557.empty and 'Fase' in _summary_preview_v1557.columns:
            _match_preview_v1557 = _summary_preview_v1557[
                _summary_preview_v1557['Fase'].astype(str) == str(_phase_preview_v1557)
            ]
            if not _match_preview_v1557.empty:
                _phase_row_preview_v1557 = _match_preview_v1557.iloc[0]
            else:
                _phase_row_preview_v1557 = _summary_preview_v1557.iloc[0]
                _phase_preview_v1557 = str(_phase_row_preview_v1557['Fase'])

        st.markdown('## Lectura clínica predictiva')
        st.caption('Orden de interpretación: **Presente → Tendencia → Probabilidad futura**. El pronóstico probabilístico se interpreta después de conocer el punto de partida y la dirección del sistema.')

        st.markdown('### 1. Estado actual · El punto de partida')
        if _phase_row_preview_v1557 is not None:
            _ca1, _ca2 = st.columns([1,3])
            _ca1.metric('Compuesto actual', f"{float(_phase_row_preview_v1557['Compuesto_actual']):.3f}")
            _ca2.caption(f"Fase/serie mostrada: **{_phase_preview_v1557}**. Este valor representa el estado fisiológico compuesto disponible hoy para esta selección.")
        else:
            st.info('Todavía no hay un compuesto actual calculable para esta selección.')

        st.markdown('### 2. Proyección del sistema · Hacia dónde va')
        if _phase_row_preview_v1557 is not None:
            _cp1, _cp2, _cp3, _cp4 = st.columns(4)
            _actual_preview_v1557 = float(_phase_row_preview_v1557['Compuesto_actual'])
            _predicted_preview_v1557 = float(_phase_row_preview_v1557['Compuesto_predicho'])
            _delta_preview_v1557 = float(_phase_row_preview_v1557['Delta_compuesto'])
            _cp1.metric('Compuesto previsto · t+1', f"{_predicted_preview_v1557:.3f}", f"Actual {_actual_preview_v1557:.3f}")
            _cp2.metric('Estado previsto', str(_phase_row_preview_v1557['Estado_previsto']))
            _cp3.metric('Δ compuesto', f"{_delta_preview_v1557:+.3f}")
            _cp4.metric('Confianza metodológica', f"{float(_phase_row_preview_v1557['Confianza_pct']):.0f}%")
            st.caption('La proyección t+1 expresa la siguiente posición esperada del sistema según el motor fisiológico activo; no equivale todavía a la probabilidad de estar mejor a un horizonte de varios días.')
        else:
            st.info('Todavía no hay una proyección t+1 calculable para esta selección.')

        st.markdown('### 3. Pronóstico probabilístico · La oportunidad futura')
        ...
        with st.expander('Probabilidad calibrada de evolución favorable a X días', expanded=True):
            ...
            if _validated:
                ...
                _pp4.metric('Prevalencia general (Train)',f"{_pm.get('prevalencia_train_pct',np.nan):.1f}%", help='Proporción de evoluciones favorables en el conjunto de entrenamiento; sirve como referencia poblacional del modelo, no como probabilidad individual.')
            else:
        ...
        if _patient_history_v1529.empty:
            ...
            st.markdown('### Detalle longitudinal del estado actual')
            ...
            st.markdown('### Detalle de proyección simultánea de todas las fases')
            if summary15.empty:
